...
export const STUBS = {
  ...
  cover: {
    ...
    countLabel: (c) => (Number.isFinite(c && c.coverTotal)
      ? `изучено экземпляров из ${c.coverTotal} найденных${Number.isFinite(c.owed) ? ` · осталось ${c.owed}` : ''}`
      : 'изучено экземпляров'),
    blocks: [
      // «Не хватает третьего и главного: осталось» was true when it was written and is not now:
      // frontierInstanceStats partitions all instances with zero residue, so «осталось» is a number
      // (141 on fix1) and the far larger bucket — 373 declined by our OWN sampling policy — is
      // separable from it. The KPI strip prints both. What remains missing is a different thing.
      { h: 'что уже считается', p: 'Всего / изучено / осталось — все три считаются, поэкземплярно, и разбиение сходится без остатка: пройдено, осталось по-настоящему, отклонено нашей же выборкой, недостижимо, перерисовалось. Числа в полосе KPI над обходом.' },
      { h: 'чего не хватает', p: 'Не хватает ЭКРАНА, разносящего «мы отказались нарочно» и «мы не смогли» по разным таблицам со своими подытогами, и — это уже пробел захвата — списка обязательств на элемент. Один клик не есть изучение: пока «изучено» означает «по нему был акт», а не «он понят», и завышать это словом «покрытие» нельзя.' },
      { h: 'что нужно', p: 'Экран — из уже собранных данных. Обязательства по элементу и отметка закрытия каждой пробы — нужен прогон с новым захватом.' },
    ],
  },
};
