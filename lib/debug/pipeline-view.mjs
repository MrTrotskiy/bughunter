...
import { rowSentence, PROTOCOL_KINDS, SPOKEN_KINDS } from './row-vocabulary.mjs';
...
// THE 13 KINDS A REAL RUN EMITS, all styled (was 3 of 13 — 825 of 1092 rows fell to the default
// «этап» badge and rendered their raw English kind name in a Russian UI). The ten added below are the
// crawler's own DECISION PROTOCOL: where it went and what it passed over (route-choice), which control
// it picked and by which rule (pick), why it pressed nothing (pick-empty), whether it may press
// (policy-verdict — a VERDICT, styled apart from a click so the two can never be confused), how a
// page-drain ended (drain-outcome), and the leftover/reopen bookkeeping. Their SENTENCE is
// row-vocabulary.mjs; here they get only a category + badge. Deliberately NOT 'navigation' (that KPI
// is wall clock in route rows, and these decisions cost ~0ms) and NOT 'error'.
export const KIND_STYLE = {
  ...
  'route-choice':   { cat: 'decision',    color: '#2593ba', badge: 'маршрут',  verb: '' },
  pick:             { cat: 'decision',    color: '#737373', badge: 'выбор',    verb: '' },
  'pick-empty':     { cat: 'decision',    color: '#737373', badge: 'пусто',    verb: '' },
  'policy-verdict': { cat: 'policy',      color: '#a1a1a1', badge: 'вердикт',  verb: '' },
  'drain-outcome':  { cat: 'decision',    color: '#737373', badge: 'итог',     verb: '' },
  retire:           { cat: 'decision',    color: '#737373', badge: 'отложено', verb: '' },
  'retire-answered':{ cat: 'decision',    color: '#737373', badge: 'ответ',    verb: '' },
  reopen:           { cat: 'decision',    color: '#737373', badge: 'возврат',  verb: '' },
  'reopen-delivered':{ cat: 'decision',   color: '#737373', badge: 'доставка', verb: '' },
  'reloc-census':   { cat: 'metric',      color: '#fe9a00', badge: 'перепись', verb: '' },
  gate:             { cat: 'policy',      color: '#a1a1a1', badge: 'решение',  verb: '' },
  'driver.open':    { cat: 'metric',      color: '#fe9a00', badge: 'старт',    verb: '' },
  'loop-terminal':  { cat: 'decision',    color: '#737373', badge: 'финал',    verb: '' },
  relogin:          { cat: 'navigation',  color: '#2593ba', badge: 'вход',     verb: '' },
};
...
export function rowStyle(row) {
  ...
  // Refine by failure class ONLY for an act — its `act.failed` tone is what makes the pipeline agree
  // with the Walk tab. A protocol row (policy-verdict, reopen, …) carries a code in `outcome` too
  // (ALLOWED / OUTWARD_REFUSED / REOPEN_UNVERIFIED), and reclassifying it would repaint the DECISION
  // badge «вердикт» as a failure chip «наружу» — a verdict must keep its own badge, never borrow one.
  const kind = row && row.kind;
  if (kind !== 'act' && kind !== 'act.failed') return base;
...
/* ------------------------------------------------------------------ zero-act drain-cycle fold */

// THE PAGE-DRAIN CYCLE IS THE NATURAL UNIT, not the row. The existing folds (foldPipeline /
// foldIdenticalRuns) demand ADJACENT same-kind rows and something always sits between them, so on the
// 1092 real rows of run fix1 they performed ZERO folds. A drain cycle — the crawler visiting a page,
// judging its controls and leaving — ends at a `drain-outcome` event that carries its OWN `acts`
// count. A cycle that produced acts:0 studied nothing new and is ONE conclusion, not a dozen rows.
//
// A cycle is delimited by its closing `drain-outcome`: the rows since the previous drain up to and
// including this one. It collapses ONLY when the drain reports acts:0 AND the window swept in NO
// act/act.failed row — the acts are the narrative and must never be hidden, and interleaved
// revisit/reopen work occasionally lands a real act inside an otherwise-empty drain window (measured:
// 1 of 54 zero-act drains on fix1 carried 6 acts). That one stays expanded; the other 53 fold.
export function drainSegments(rows) {
  const out = []; let buf = [];
  for (const r of Array.isArray(rows) ? rows : []) {
    if (!r || typeof r !== 'object') continue;
    buf.push(r);
    if (r.kind === 'drain-outcome') { out.push(buf); buf = []; }
  }
  if (buf.length) out.push(buf);
  return out;
}
const isZeroActDrain = (seg) => {
  const d = seg[seg.length - 1];
  return !!(d && d.kind === 'drain-outcome' && d.payload && d.payload.acts === 0);
};
const segHasAct = (seg) => seg.some((r) => r && (r.kind === 'act' || r.kind === 'act.failed'));

// Collapse every barren zero-act drain cycle into one conclusion row. Time is CONSERVED (the merged
// durMs is the segment sum, exactly as foldPipeline/foldIdenticalRuns), and `members` keeps every
// original row so the fold is a reading aid the operator expands, never a filter. The merged row
// carries NO `outcome` and a non-foldable kind, so neither earlier fold re-folds it.
export function foldZeroActCycles(rows) {
  const out = [];
  for (const seg of drainSegments(rows)) {
    if (!isZeroActDrain(seg) || segHasAct(seg)) { for (const r of seg) out.push(r); continue; }
    const drain = seg[seg.length - 1];
    const merged = {
      ...seg[0], kind: 'drain-outcome', foldKey: null, foldKind: 'cycle',
      route: drain.route != null ? drain.route : seg[0].route,
      cycleRoute: drain.route || (drain.payload && drain.payload.route) || null,
      payload: drain.payload || seg[0].payload || {},
      count: seg.length, members: seg, outcome: null, stages: [],
      durMs: 0, declaredMs: 0, idleMs: 0, requests: 0,
    };
    for (const r of seg) {
      merged.durMs += Number.isFinite(r.durMs) ? r.durMs : 0;
      merged.declaredMs += Number.isFinite(r.declaredMs) ? r.declaredMs : 0;
      merged.idleMs += Number.isFinite(r.idleMs) ? r.idleMs : 0;
      merged.requests += Number.isFinite(r.requests) ? r.requests : 0;
    }
    out.push(merged);
  }
  return out;
...
// The run-level conclusion the banner states: how many page visits drained with ZERO actions, and
// what share of wall clock they cost. Counted from the drain-outcome payloads directly (`acts:0`),
// so the number is the crawler's own, not a reconstruction; time is summed over each zero-act drain's
// window (its rows since the previous drain), consistent with drainSegments and with the fold.
export function zeroActCycleStats(rows) {
  const list = Array.isArray(rows) ? rows.filter((r) => r && typeof r === 'object') : [];
  const totalMs = list.reduce((s, r) => s + (Number.isFinite(r.durMs) ? r.durMs : 0), 0);
  let drains = 0, zeroDrains = 0, zeroMs = 0;
  for (const seg of drainSegments(list)) {
    const d = seg[seg.length - 1];
    if (!d || d.kind !== 'drain-outcome') continue;
    drains++;
    if (d.payload && d.payload.acts === 0) {
      zeroDrains++;
      for (const r of seg) zeroMs += Number.isFinite(r.durMs) ? r.durMs : 0;
    }
  }
  return { drains, zeroDrains, zeroMs, totalMs, pct: totalMs > 0 ? (zeroMs / totalMs) * 100 : 0 };
}

// THE DRIVER, DERIVED FROM THE EVENTS — never hardcoded. The node loop (script) emits pick /
// route-choice / drain-outcome; the agent path emits `observe` (the LLM's written verdict). A run with
// observe events was driven by the AGENT; one with pick/route-choice and no observe by the SCRIPT;
// neither, unknown. The old note stamped «весь маршрут выбирает скрипт» into EVERY run — a lie on an
// agent-path trail. fix1 has 187 pick and 0 observe → СКРИПТ, and it says so because the data says so.
export const DRIVER_LABEL = {
  agent: 'Этот прогон вёл АГЕНТ — контролы выбирала модель, а не скрипт',
  script: 'Этот прогон вёл СКРИПТ — детерминированный обход, модель ничего не решала',
  unknown: 'драйвер этого прогона не определён',
};
export function deriveDriver(rows) {
  let observe = false, script = false;
  for (const r of Array.isArray(rows) ? rows : []) {
    if (!r) continue;
    // A RECORDED driver.open is authoritative — the run SAYS what drove it, so we no longer infer.
    // Inference stays as the fallback for older trails written before the driver record existed.
    if (r.kind === 'driver.open' && r.payload && (r.payload.driver === 'agent' || r.payload.driver === 'script')) {
      return { driver: r.payload.driver, label: DRIVER_LABEL[r.payload.driver] };
    }
    if (r.kind === 'observe') observe = true;
    else if (r.kind === 'pick' || r.kind === 'route-choice') script = true;
  }
  const driver = observe ? 'agent' : script ? 'script' : 'unknown';
  return { driver, label: DRIVER_LABEL[driver] };
}

// PERMIT GATES FOLD INTO THEIR ACT (Stage-6 render fix). A `gate{decision:'permit'}` fires once per
// actStep (~200/run) and sits immediately before the `act`/`act.failed` it cleared — as its own row it
// floods the timeline (+18% rows, none foldable by the other three passes) and strands its resolve/
// ownership cost on a separate line rendered as idle. Merge it into the following act of the SAME
// templateId: time is CONSERVED (the permit's durMs — the resolve+ownership gap — moves onto the act),
// its `resolve` stage (scrub-math synthesizes it from the top-level resolveMs) PREPENDS the act's stages
// so the ~30%-of-wall-clock cost is CREDITED not idle, and its ownership/floor ride the act row for the
// inspector. A REFUSE/SKIP gate is NOT folded — it is the reason «почему не нажал», a first-class row of
// its own (and keeps its own resolve stage). The merged act carries `permitFolded` so no later fold
// re-folds it. Runs FIRST (innermost) so the other three passes see merged act rows, not permit+act pairs.
export function foldPermitGates(rows) {
  const out = [];
  const list = Array.isArray(rows) ? rows : [];
  for (let i = 0; i < list.length; i++) {
    const r = list[i];
    const next = list[i + 1];
    if (r && r.kind === 'gate' && r.payload && r.payload.decision === 'permit'
      && next && (next.kind === 'act' || next.kind === 'act.failed')
      && next.templateId != null && next.templateId === r.templateId) {
      const durMs = (Number.isFinite(next.durMs) ? next.durMs : 0) + (Number.isFinite(r.durMs) ? r.durMs : 0);
      const stages = [...(r.stages || []), ...(next.stages || [])].map((s) => ({ ...s }));
      const declaredMs = stages.reduce((sum, s) => sum + s.ms, 0);
      ...
      out.push({
        ...
        overDeclared, stages,
        permitFolded: true,
        permit: { ownership: r.payload.ownership ?? null, resolveMs: r.payload.resolveMs ?? null, floor: r.payload.floor ?? null },
      });
      i++; // the act row is consumed into the merged row
      continue;
    }
    out.push(r);
  }
...
// The folds, in the one order that works: PERMIT gates merge into their act first (so the rest see act
// rows, not permit+act pairs), navigation patterns next (they collapse route rows that would otherwise
// sit between two failures and break a genuine repeat run), identical outcomes third, and the zero-act
// drain cycle LAST — it operates on the already-folded stream (its window may contain a folded route or
// repeat row, kept as a member) and its output rows must be final, because foldPipeline's else-branch
// would otherwise overwrite their count/members. Rows already collapsed by an earlier fold carry count>1
// and no re-foldable `outcome`, so no fold re-folds another's row.
export function foldAll(rows) { return foldZeroActCycles(foldIdenticalRuns(foldPipeline(foldPermitGates(rows)))); }
const isFoldGroup = (r) => !!(r && r.count > 1 && (r.foldKind === 'repeat' || r.foldKind === 'cycle'));
...
// subtree (`Control A Control B Control C (0)No dataNo results found`). displayName
...
export function rowTitle(row, graph) {
  // A protocol row's title IS its decision sentence, which runs longer than a control name — clip it
  // wider (CSS ellipsis still trims the visible column; rowTitleFull keeps the whole for the tooltip).
  const max = row && PROTOCOL_KINDS.has(row.kind) ? 120 : 72;
  return displayName(rowTitleFull(row, graph), max).text;
}
...
export function rowTitleFull(row, graph) {
  ...
  // A COLLAPSED ZERO-ACT DRAIN CYCLE states its conclusion, not a member's name: the crawler visited
  // this page and left without pressing anything, because there was nothing unexplored left to press.
  if (row && row.foldKind === 'cycle') {
    const n = row.count || 1;
    const route = row.cycleRoute || '(адрес не записан)';
    const tail = n > 1 ? ` · свёрнуто ${n} ${plural(n, ['строка', 'строки', 'строк'])}` : '';
    return `Заход на ${route} — 0 действий (неизученных контролов не было)${tail}`;
  }
  // A PROTOCOL row (route-choice / pick / policy-verdict / drain-outcome / …) carries no useful
  // WHO/WHERE of its own — the sentence IS what it did and why, read from its payload. The badge
  // already carries the kind, so the label is the sentence alone (a ×N for a folded protocol row,
  // which today never folds — the guard is cheap and keeps the two paths identical).
  if (row && PROTOCOL_KINDS.has(row.kind)) {
    const s = rowSentence(row.kind, row.payload || {});
    return row.count > 1 ? `${s} ×${row.count}` : s;
  }
...
/* the run-level conclusion, STATED above the list — the zero-act-cycle headline (a border-left
   accent, no filled background, house rule) so the operator reads the point before the rows */
.pv-banner { padding:8px 14px; font-size:12px; color:var(--fg); background:var(--pv-card);
             border-bottom:1px solid var(--pv-line); border-left:2px solid var(--warn); flex:0 0 auto; }
.pv-banner b { color:var(--fg); font-variant-numeric:tabular-nums; }
...
/* the row's one-line human sentence — what the crawler did and why — sits directly under the title,
   above the raw fields, in the operator's language (row-vocabulary.mjs) */
.pv-say { color:var(--fg); font-size:12px; line-height:1.5; margin:2px 0 8px; }
...
function kpiStrip(k) {
  ...
  return [
    ...
    // decisionMs is null on every trail on disk — no writer stamps a model-decision stage — so the
    // value is honestly «—» under a label that SAYS it was not measured, never a fabricated 0. The
    // day a writer emits such a stage decisionMs goes non-null and this lights up by itself.
    kpi(k.decisionMs == null ? '—' : fmtMs(k.decisionMs),
      k.decisionMs == null ? 'решения модели не измерялись' : 'решения модели',
      'ни один писатель трейла не размечает стадию решения модели, поэтому здесь «—», а не ноль'),
  ].join('');
}

// The zero-act-cycle headline. Stated only when there were drains AND at least one was barren — a
// banner that always renders makes no claim. The % is shown only when wall clock is measured; with
// no timings the count and the share of cycles still stand, never a fabricated number.
function bannerHtml(cyc) {
  if (!cyc || !(cyc.drains > 0) || !(cyc.zeroDrains > 0)) return '';
  const share = cyc.totalMs > 0
    ? ` — <b>${Math.round(cyc.pct)}%</b> времени прогона`
    : '';
  return `<div class="pv-banner"><b>${cyc.zeroDrains}</b> из <b>${cyc.drains}</b> ${plural(cyc.drains, ['захода', 'заходов', 'заходов'])} не дали ни одного действия${share}. Эти строки свёрнуты в один вывод — раскрываются по клику.</div>`;
}
...
function rowHtml(row, i, scale, selected, ctx) {
  ...
  const rep = isFoldGroup(row)
    ? `<span class="pv-rep" data-exp="${i}" title="${esc(`${row.count} свёрнутых строк — раскрыть`)}">${expanded ? '▾' : '▸'} ×${esc(row.count)}</span>` : '';
...
function inspectorHtml(row, ctx) {
  ...
  const folded = row.count > 1 && !row.foldKind
  ...
  const repeated = row.foldKind === 'repeat' && row.count > 1
    ? `<h5>свёрнутая серия</h5>`
      ...
      + (row.members.length > 12 ? `<div class="pv-why">…и ещё ${row.members.length - 12}</div>` : '')
    : '';
  // A collapsed zero-act drain cycle: the visit and everything the crawler weighed on it, none of
  // which produced a click. Members stay listed so the fold is a reading aid the operator can open.
  const cyclefold = row.foldKind === 'cycle' && row.count > 1
    ? `<h5>свёрнутый заход</h5>`
      + row.members.slice(0, 12).map((m) => `<div class="pv-kv"><span class="pv-mono">${esc(String(rowTitle(m, idGraph)).slice(0, 44))}</span><b>${esc(fmtMs(m.durMs))}</b></div>`).join('')
      + (row.members.length > 12 ? `<div class="pv-why">…и ещё ${row.members.length - 12}</div>` : '')
    : '';
  // The one-line sentence for THIS row, in the operator's words — what the crawler did and why, read
  // from the event's own payload. Shown for every kind the vocabulary speaks (all 13 that occur); a
  // kind it does not know renders no lead rather than a bare English «Событие …».
  const say = row && SPOKEN_KINDS.has(row.kind) && row.payload
    ? `<div class="pv-say">${esc(rowSentence(row.kind, row.payload))}</div>` : '';
  return `<h4 title="${esc(rowTitleFull(row, idGraph))}">${esc(rowTitle(row, idGraph))}</h4>
    ${say}${verdictBlock(row, b)}
...
export function mountPipeline(el, { graphFor, identityGraph } = {}) {
  ...
  el.classList.add('pv');
  let rows = [], rawRows = [], sel = 0;
  const expanded = new Set();   // fold-group row keys the operator opened
  ...
  const paint = () => {
    ...
    // A fold group the operator opened (a repeat SERIES or a zero-act drain CYCLE) is drawn as its
    // members, in place. The fold is a reading aid, never a filter: expanding restores every row.
    const view = [];
    rows.forEach((r, i) => {
      if (isFoldGroup(r) && expanded.has(rowKey(r))) {
    ...
    // The driver is DERIVED from the events (deriveDriver), never hardcoded — an agent-path run would
    // make «весь маршрут выбирает скрипт» a lie. The banner states the zero-act-cycle conclusion.
    const driver = deriveDriver(rawRows.length ? rawRows : rows);
    el.innerHTML = `<div class="pv-kpis">${kpiStrip(pipelineKpis(rows))}</div>
      ${bannerHtml(zeroActCycleStats(rawRows.length ? rawRows : rows))}
      <div class="pv-note">Каждая строка — один шаг конвейера: что сделано, за сколько и из чего сложилось это время.
        <b>${esc(driver.label)}</b>: неучтённое время помечено как «${esc(UNEXPLAINED_LABEL)}», а не как работа модели и не как навигация.
  ...
  return {
    ...
    update(pipelineRows) {
      // Keep the UNfolded rows so the banner/driver count drains and observe events over the whole
      // trail — folding absorbs the barren drain rows into cycle rows and would undercount them.
      rawRows = Array.isArray(pipelineRows) ? pipelineRows : [];
