...
// The three folds, in the one order that works: navigation patterns first (they collapse route rows
// that would otherwise sit between two failures and break a genuine repeat run), identical outcomes
// second, and the zero-act drain cycle LAST — it operates on the already-folded stream (its window may
// contain a folded route or repeat row, kept as a member) and its output rows must be final, because
// foldPipeline's else-branch would otherwise overwrite their count/members. Rows already collapsed by
// an earlier fold carry count>1 and no re-foldable `outcome`, so no fold re-folds another's row.
export function foldAll(rows) { return foldZeroActCycles(foldIdenticalRuns(foldPipeline(rows))); }
