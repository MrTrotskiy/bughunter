...
import { frontierInstanceStats, frontierInstanceBuckets } from '../recon/frontier.mjs';
...
function loopbackHost(host, port) { // eslint-disable-line no-unused-vars -- port kept for call-site parity
  if (!host) return false;
  const h = String(host).toLowerCase();
  // The PORT is whatever an SSH tunnel / port-forward chose on the operator's side and is NOT a security
  // property — anti-DNS-rebinding rests entirely on the HOSTNAME being a loopback literal (a rebinding
  // page cannot make a browser send Host: 127.0.0.1 / localhost / [::1]). Matching the bound port too
  // spuriously 403'd a forward whose local port differs from the server's (the "forbidden host" the
  // operator hit). So match the loopback hostname and ignore the port; a foreign host is still refused.
...
// trace.mjs gzips graph snapshots on write (a ~1 MB graph compresses ~20x, so a 200-act run's
// graph/ dir is ~6 MB, not ~126 MB). The framing is the gzip magic 0x1f8b — JSON text never starts
// with those bytes, so a legacy uncompressed snapshot is told apart and read as-is. `isGzip` gates
// both the in-process parse below AND the served content-encoding, so the two never disagree.
function isGzip(buf) { return buf.length >= 2 && buf[0] === 0x1f && buf[1] === 0x8b; }
// Read a graph snapshot as parsed JSON, inflating a gzip-framed one. Server-side only (the coverage
// stats need the parsed graph); the browser-facing route serves the compressed bytes untouched.
function readGraphSnapshot(file) {
  const buf = fs.readFileSync(file);
  return JSON.parse((isGzip(buf) ? zlib.gunzipSync(buf) : buf).toString('utf8'));
}
...
// Parse the newest snapshot ONCE and derive BOTH the instance-level split and its per-bucket drill-down
// (frontierInstanceBuckets — the same rule, so the drill-down counts sum to the split counts, and the
// viewer never re-derives a cap). Memoized together on file+mtime so the ~1 MB graph is parsed once per
// snapshot, whichever of instanceStatsFor / instanceBucketsFor asks first.
function statsAndBucketsFor(id) {
  let names;
  try { names = fs.readdirSync(path.resolve(runDir(id), 'graph')); } catch { return null; }
  ...
  let out = null;
  try {
    const graph = readGraphSnapshot(file);   // gunzips a gzip-framed snapshot; parses a legacy one
    if (graph && graph.elements) {
      ...
      const stats = { ...frontierInstanceStats(graph), instances, templates: Object.keys(graph.elements).length, atSeq: bestSeq };
      out = { stats, buckets: frontierInstanceBuckets(graph) };
    }
...
function instanceStatsFor(id) { const o = statsAndBucketsFor(id); return o ? o.stats : null; }
function instanceBucketsFor(id) { const o = statsAndBucketsFor(id); return o ? o.buckets : null; }
...
// A graph snapshot is stored gzip-framed (trace.mjs). Serve the compressed bytes AS-IS with
// `content-encoding: gzip` — the browser's fetch() inflates transparently, so zero server CPU on
// the hot scrubber path, and the page's JSON.parse sees the same bytes it always did. A legacy
// uncompressed snapshot carries no encoding header. content-length is the on-the-wire (compressed)
// length, per HTTP; the client inflates after receiving it.
function serveGraphSnapshot(res, file) {
  fs.readFile(file, (err, buf) => {
    if (err) { sendJson(res, 404, { ok: false, error: 'not found' }); return; }
    const extra = isGzip(buf) ? { 'content-encoding': 'gzip' } : {};
    res.writeHead(200, { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'no-store', 'content-length': buf.length, ...SEC_HEADERS, ...extra });
    res.end(buf);
  });
}
...
export function handle(req, res, { port, token }) {
  ...
  // The «Покрытие» screen's TEXT module (coverage-view.mjs, imports from walk-view) — same rule again:
  // code, not run data, so token-free, FIXED basename. Without this branch the page's
  // `import './coverage-view.mjs'` 404s under `script-src 'self'` and the coverage screen mounts nothing.
  if (seg.length === 1 && seg[0] === 'coverage-view.mjs') { serveFile(res, path.join(HERE, 'coverage-view.mjs'), 'text/javascript; charset=utf-8'); return; }
  ...
  if (seg[0] === 'api') {
    ...
    if (seg[1] === 'runs' && id) {
      ...
      if (seg.length === 3) {
        ...
        // `instanceBuckets` is the per-bucket drill-down of `instanceStats` (which controls the sampling
        // policy declined, which the app broke, which churned) — the «Покрытие» screen's evidence. Same
        // rule, same snapshot; shipped as a sibling, not merged into stats. null when no graph snapshot.
        sendJson(res, 200, { ok: true, run, events: readEvents(id), instanceStats: instanceStatsFor(id), instanceBuckets: instanceBucketsFor(id) }); return;
      }
      // /api/runs/:id/graph/:seq → a graph snapshot (either filename shape, newest match), gzip-framed
      // and served content-encoded so the browser inflates it transparently.
      if (seg[3] === 'graph' && SEQ.test(seg[4] || '')) {
        const f = graphSnapshotFile(id, seg[4]);
        if (!f) { sendJson(res, 404, { ok: false, error: 'not found' }); return; }
        serveGraphSnapshot(res, f); return;
      }
