...
// THE DRIVER RECORD (`driver.open`). Who drove this run — the script (recon-run's node loop) or the agent
// (the recon subagent calling whats-new) — plus the argv-derived flags (url, steps, mode). Before this the
// viewer INFERRED the driver from which kinds happened to appear (deriveDriver), and hardcoded the answer
// when the inference was thin; a run now SAYS what drove it. Idempotent: emitted at most once per run
// (driver.open is always among the first events, so a cheap head-scan dedups it across the agent path's
// many whats-new processes joined only by BUGHUNTER_RUN_ID). Best-effort head read; a fresh run has none.
export function driverOpen(runId, descriptor = {}) {
  if (!runId) return;
  ...
  try {
    ...
    try {
      ...
      if (buf.toString('utf8', 0, n).includes('"kind":"driver.open"')) return; // already recorded
    } finally { fs.closeSync(fd); }
  } catch { /* no file yet — this is the first event, emit below */ }
  try { traceEvent(runId, 'driver.open', descriptor); } catch { /* a driver record must never break the run */ }
}
...
// Snapshot the whole graph.json as it stands after step `seq`, so the admin can scrub graph
// GROWTH over the walk. GZIPPED on write: a full-copy snapshot was ~1 MB and this fires on every
// act/route event (200+ per run), so a run's graph/ dir grew to ~126 MB of near-identical copies;
// a graph compresses ~20x (measured 1042067 -> 49112 B), so the same run's snapshots are now ~6 MB.
// The bytes stay JSON after inflation (bit-identical to the old copy), so the scrubber is unchanged —
// admin-server serves them content-encoded so the browser's fetch() inflates transparently, and
// gunzips in-process where it parses the newest snapshot for the coverage stats.
// The filename carries the PID because `seq` is only a per-process hint (see traceEvent): two
// agent-path CLIs can derive the same seq, and `${seq}.json` alone meant the second write silently
// DESTROYED the first's snapshot. The `.json` name is KEPT (gzip lives in the bytes, framed by its
// own 0x1f8b magic — JSON never starts with those bytes, so a reader tells the two apart), so the
// admin-server filename resolver contract (`${seq}-<pid>.json` / legacy bare `${seq}.json`) is
// untouched. DEBUG-ONLY and gated on `runId`: a Node-side file write under `__idle__`, no page
// interaction, no causal window — attribution and coverage are unaffected.
export function snapshotGraph(runId, seq) {
  ...
  try {
    ...
    fs.writeFileSync(path.join(runDir(runId), 'graph', `${seq}-${process.pid}.json`), gz);
  } catch {}
}
