...
export function failurePanel(s, ex, oc, ctx = {}) {
  ...
  // REVEAL-REPLAY OUTCOME (L1) — only the INFORMATIVE case: the recovery pass reopened the container
  // successfully and acted, and it STILL failed. Without this line a reveal-required NO_INSTANCE reads the
  // same whether the container never opened or opened-and-the-control-still-would-not-act; the distinction is
  // the whole answer to "why did we not reach it". A main-pass failure (`replayed:false`) adds nothing here.
  const rr = s.revealReplay && s.revealReplay.replayed === true
    ? `<div class="ocnote">контейнер переоткрыли (${esc(s.revealReplay.rung || 'reopen')}) — контрол нашёлся при переоткрытии, но акт всё равно не прошёл</div>`
    : '';
  ...
  return `<div class="ocard">
    ...
    <div class="ocsec"><h6>где это было</h6><div class="v">${where}${reveal}${rr}</div></div>
