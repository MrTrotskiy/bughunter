...
export function deriveSteps(events) {
  ...
  return acts.map((e) => {
    ...
    const step = {
      ...
      shots: p.shots || null,
      // THE RESOLVER'S OWN RECORD OF THE ATTEMPT, and the field this projection used to drop. Every
      // `act.failed` carries `target: {selector, attempts[6], hadRevealPath, locatorType}` — the six
      // per-strategy records (`{strategy, ran, raw, visible, sameTemplate}`) the locator resolver
      // actually ran. deriveSteps did not carry it while its sibling derivePipeline (~L160) did, so
      // the walk's failure card printed «список попыток не записан» on 39 of 39 failures of run
      // `fix1` while the Конвейер tab, reading the same events through the other projection, had the
      // list — and the two tabs contradicted each other about `hadRevealPath` on the same act (walk:
      // «нет отметки о пути», pipeline: «путь был записан», true on all 5 NO_INSTANCE acts). One
      // dropped field, three false sentences: the writer wrote it, the projection lost it, and the
      // renderer printed its "not recorded" branch. Copied verbatim, exactly as derivePipeline does.
      target: p.target && typeof p.target === 'object' ? p.target : null,
      // WHETHER REVEAL-REPLAY WAS INVOLVED (L1) — a recovery act that failed after its container reopened
      // carries `{replayed:true, ok, rung}`, so the walk can say "reopened, acted, still failed" instead of
      // leaving a reveal-required NO_INSTANCE indistinguishable from a live-state miss. Additive; absent on
      // an ordinary main-pass failure.
      revealReplay: p.revealReplay && typeof p.revealReplay === 'object' ? p.revealReplay : null,
      // The DOM skeleton written for a failed act (`skel/<stem>.json`, lib/graph/dom-skeleton.mjs) —
      // the schematic stand-in for the key-frame that structurally cannot exist on the failure path.
      // admin-server has served /api/runs/:id/skel/:file since it was built; without this line the
      // viewer has no way to ask for it.
...
export function derivePipeline(events) {
  ...
  return list.map((e) => {
    ...
    // A gate row carries its ownership-proof cost at payload top-level (`resolveMs`), NOT in `timings`
    // — so credit it as a `resolve` stage, else the ~30%-of-wall-clock resolve+ownership work the gate
    // row legitimately owns falls into idleMs («не измерено») even though the datum is captured. The
    // PERMIT gate is folded into its act by the render (foldPermitGates); a standalone refuse/skip gate
    // keeps this stage of its own so its row is explained, not blank.
    if (Number.isFinite(p.resolveMs) && !stages.some((s) => s.name === 'resolve')) {
      stages.unshift({ name: 'resolve', ms: p.resolveMs });
    }
    ...
    return {
      ...
      // The event's own payload, carried by REFERENCE (no copy) so the «Конвейер» tab can render the
      // per-kind decision sentence (row-vocabulary.mjs) — a route-choice names what it passed over, a
      // pick the ranking rule, a policy-verdict the gate. The promoted fields above stay for the code
      // that already reads them; this is the fallback for everything a new kind's sentence needs.
      payload: p,
    };
  });
}
