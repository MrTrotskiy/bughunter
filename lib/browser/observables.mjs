...
export async function domFingerprint(page, { scope = null } = {}) {
  return page.evaluate((sel) => {
    ...
    walk(root, 0);
    // STATE IS AN OBSERVABLE, and a tag census cannot see it. Switching a tab, expanding an accordion,
    // ticking a checkbox or selecting a row changes ARIA/state ATTRIBUTES while leaving the tag/depth
    // census byte-identical — so the act recorded `inert`, "the page did nothing", when in fact the page
    // did exactly what the control is for. Measured: 22% of probed controls had only inert rows, and the
    // knowledge ladder deliberately refuses to call those understood, so they became a hard ceiling on
    // coverage that no amount of extra acts could lift.
    // Counted, never keyed by content: these are COUNTS of state-bearing elements, so a live feed
    // rewriting its text still hashes identically (the same reason text is excluded from the census).
    // The URL joins the signature for the same reason — an in-app route change is the most consequential
    // observable there is, and on a client-routed app it can happen with no structural change at all.
    // WHICH elements hold the state, not HOW MANY. Counting was the obvious first cut and it is blind to
    // the commonest case of all: switching a tab moves `aria-selected` from one element to another, so the
    // count stays 1 and the signature is unchanged. Each match therefore contributes its POSITION —
    // tag plus index among siblings — which is content-free (a feed rewriting its text does not move
    // anything) and cheap. The URL joins for the same reason: on a client-routed app a section change can
    // happen with no structural change at all.
    const stateSig = (sel) => {
      try {
        return [...document.querySelectorAll(sel)]
          .slice(0, 40)   // bounded: a huge table of checkboxes must not make this the dominant cost
          .map((e) => `${e.tagName}${e.parentNode ? [...e.parentNode.children].indexOf(e) : -1}`)
          .join('.');
      } catch { return ''; }
    };
    const states = [
      `sel=${stateSig('[aria-selected="true"]')}`,
      `exp=${stateSig('[aria-expanded="true"]')}`,
      `chk=${stateSig('[aria-checked="true"], input:checked')}`,
      `open=${stateSig('[data-state="open"], [data-state="active"], [data-state="checked"]')}`,
      `press=${stateSig('[aria-pressed="true"]')}`,
      `dis=${stateSig('[disabled], [aria-disabled="true"]')}`,
      `url=${location.pathname}${location.search}${location.hash}`,
    ];
    let h = 2166136261 >>> 0;
    for (const k of [...counts.keys()].sort().concat(states)) {
      const s = counts.has(k) ? `${k}=${counts.get(k)}` : k;
