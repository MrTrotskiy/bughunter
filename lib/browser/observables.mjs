...
export async function readOutcome(page, { scope = null } = {}) {
  const dom = await page.evaluate((sel) => {
    ...
    // SUCCESS / FAILURE ANNOUNCEMENTS. On a target with no validation at all this is the only channel,
    // and it carries both outcomes — the class distinguishes them where the framework marks it.
    const liveRegions = [...document.querySelectorAll('[role=alert],[role=status],[aria-live],.ant-message-notice,.ant-notification-notice')]
