...
export async function attachInitiatorTracker(page) {
  const inert = { verdictFor: () => null, reset: () => {}, detach: async () => {} };
  let session;
  try {
  ...
  return {
    ...
    // Clear accumulated per-path verdicts WITHOUT dropping the CDP arming (async call
    // stacks stay enabled). A reused page (persistent session, re-navigated per act) must
    // reset between acts: the `anyForeground` flag is sticky, so a path a PRIOR act
    // click-rooted would otherwise keep suppressing the initiator's timer-rejection for a
    // background poll on that same path in a LATER act's window — a phantom causal edge.
    reset() { verdicts.clear(); },
    async detach() {
