...
import { pathKey, originOf } from './initiator.mjs';
  ...
  return {
    ...
    onRequest(params) {
      ...
      const entry = {
        ...
        // The CDP url is ABSOLUTE even when the page passed fetch a relative string, so this is the
        // ONLY place the host is knowable — the fire ring records whatever fetch got. Keeping only
        // `pathname` here is what made every request record host-less downstream. ORIGIN, not the
        // full url: security review [L1] forbids persisting a raw url (secret-in-query), and the
        // query keys already survive masked in `urlPattern`.
        origin: originOf(url),
        ...
        endTs: null,
        // WALL-CLOCK epoch ms, deliberately NOT startTs: `timestamp` is CDP's monotonic domain time
        // (an arbitrary epoch), fine for deriving a duration and useless for lining a request up
        // against a trail event, whose `ts` is Date.now(). `wallTime` is CDP's seconds-since-epoch,
        // present on requestWillBeSent only; fall back to the local clock when it is absent.
    ...
    takeResponse(method, pathname) {
      ...
      for (let i = Math.max(0, causeCursor); i < ledger.length; i++) {
        ...
        return {
          ...
          // origin + startedAt ride the SAME additive join as status/mime — they answer "which host
          // was this, and when" for a record that otherwise carries only a path-shaped pattern.
          origin: e.origin ?? null, startedAt: e.startedAt ?? null,
