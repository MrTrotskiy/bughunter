...
import { attachInitiatorTracker, pathKey, originOf } from './initiator.mjs';
...
// 6000ms, not 3000. The bound exists because a persistent-connection app never truly goes idle, so it
// must stay bounded — but 3s was BELOW the target's own response time: the first target's user search answers in
// 2.5-3.1s, so the snapshot was taken while the list was still an antd skeleton and the ~17 result rows
// were never captured. That silently broke the add-friend flow at its second step. The cost of the wider
// bound is per-act latency on a slow endpoint; the cost of the narrow one was losing the content entirely.
export async function waitSettled(page, { timeout = 6000, interval = 60 } = {}) {
...
export async function endCause(page, seq0, cause) {
  ...
  if (tracker && tracker.takeResponse) {
    const pending = [];
    for (const f of kept) {
      // ORIGIN falls back to the fire's own url: the ring usually holds a RELATIVE string (originOf
      // → null), but an absolute fetch() url is knowable without the ledger, so a fire the ledger
      // never matched still carries a host rather than none. Overwritten by the ledger's absolute
      // view below when the join succeeds.
      const own = originOf(f.url);
      if (own) f.origin = own;
      const meta = tracker.takeResponse(f.method, pathKey(f.url));
      if (!meta) continue;
      if (meta.origin) f.origin = meta.origin;
      f.startedAt = meta.startedAt;
