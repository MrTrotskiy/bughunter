...
// 7: reveal paths became PARENT POINTERS in the reveal tree instead of the accumulated act log. Every path
// written under 6 is a session history — measured, one carried a breadcrumb that navigated back OUT of the
// container opened two hops earlier, followed by three mutually exclusive modal openers, and another carried
// the same template twice ([35, 35]). First-reveal-wins means such a path is frozen forever, so the writer
// fix alone cannot repair a carried-over graph; the bump is what retires them. Routes survive the reset
// (re-armed to pending) — they are URL-keyed metadata, not element identity.
// 8: dom-snapshot now strips the transient `-dragged`/`-dragging` (react-draggable) state suffix from the
// template selector — an INTENTIONAL identity re-key (same class as the INC.6 state-class strip), so a
// draggable control that fragmented into a dragged-vs-never-dragged template pair collapses to one. A graph
// carried under 7 still holds the fragmented keys, so it is reset rather than co-mingled.
// 9: dom-snapshot also strips the `-hidden` VISIBILITY state suffix (`ant-dropdown-hidden`) — same class of
// re-key: an open-vs-shut dropdown fragmented into two templates for one control, while visibility is already
// recorded in the additive `visible` field. A graph carried under 8 holds the split keys, so it is reset.
...
// Bound on the distinct origins recorded per request node (see addTrigger).
const ORIGINS_CAP = 8;
...
// IS THIS PATH SEGMENT A PARAMETER SLOT? Routers disagree on the sigil and this rule is expressed in
// several places, so it lives here once. Some routers write `:id` (React Router / Express); others write `$id` — which the
// live target uses — writes `$id` (file-based routing turns `<entity>.$id.tsx` into `/<entity>/$id`).
// Reading only `:` meant a `$`-sigil param route was classified STATIC: `/<entity>/$id` was seeded as a
// concrete pending route, navigated to LITERALLY, answered by the SPA shell with a 200, and recorded as a
// visited section holding zero controls. Measured on run goal1: 4 such routes, including
// the richest page in the application, "visited" without ever existing.
export const isParamSegment = (seg) => {
  const s = String(seg || '');
  return s.startsWith(':') || s.startsWith('$');
};

// Does this path carry a parameter slot in ANY segment?
export const isParamPath = (p) => String(p || '').split('?')[0].split('/').filter(Boolean).some(isParamSegment);

export function matchParamPattern(graph, concreteRoute) {
  const routes = graph.routes || {};
  const cSegs = String(concreteRoute).split('?')[0].split('/').filter(Boolean);
  if (!cSegs.length || cSegs.some(isParamSegment)) return null;
  ...
  for (const node of Object.values(routes)) {
    ...
    if (pSegs.length !== cSegs.length || !pSegs.some(isParamSegment)) continue;
    let ok = true; let literals = 0;
    for (let i = 0; i < pSegs.length; i++) {
      if (isParamSegment(pSegs[i])) continue;               // a param slot matches any concrete segment
...
// ═══ STATE IS NOT A DECLARATION ══════════════════════════════════════════════════════════════════════
//
// `fieldFacts` merges write-once-then-fill (see mergeSnapshot): an existing non-null fact is never
// overwritten, so the first honest reading stands. That is right for what a field DECLARES about itself
// and wrong for what it happens to be DOING right now.
//
// The split is by DOM semantics, not by taste. `maxLength` / `minLength` / `pattern` / `min` / `max` /
// `step` / `inputMode` / `kind` reflect CONTENT ATTRIBUTES the author wrote in the markup, and `label` /
// `placeholder` / `hint` / `options` are read off the authored markup around the field — all properties of
// the control. `required` is nominally an attribute too and stays write-once for a STRONGER reason: our
// reading of it is a three-way OR that includes AntD's `.ant-form-item-required` wrapper class, which an
// early snapshot can catch before the wrapper has rendered. Write-once-then-fill exists precisely so a
// later, fuller reading can ADD it; making it re-readable would let a snapshot that misses the wrapper
// UNSET it and silently delete the `fill-empty` obligation a required field owes (knowledge.batteryFor).
//
// `disabled` and `readOnly` are neither. Both are live IDL PROPERTIES the application flips as the user
// works — a wizard field disabled until the previous step completes, a Save disabled until the form is
// dirty, a profile field readOnly until "Edit" is pressed. Write-once made the FIRST sighting a permanent
// verdict: measured on runs raw3 and hunt1, "Enter your email id" (tpl 1141) and "Group Name" (tpl 1020,
// a field inside a create-group modal) each latched `disabled:true` and were written off for the rest of
// the run — 8 DISABLED acts apiece, the second-largest outcome class of raw3. It cost twice over:
// stateful-step's probe selection (`!facts.disabled && !facts.readOnly`) then declined to give either
// field a fill obligation at all, and docs/GOAL.md's finding — "a control disabled where a working one is
// expected" — became indistinguishable from "disabled because the wizard has not reached this step".
//
// READONLY IS DELIBERATELY IN, and it is the one that looks wrong. This project already leans on it as a
// declaration (dom-snapshot's own note: "Event Type is a readonly select, value comes from a list, never
// typed") — but that permanence is a property of the ANTD WIDGET, not of the attribute: `el.readOnly` is
// toggled by ordinary application code exactly like `el.disabled`, and the DOM draws no distinction
// between them. Re-reading is strictly more accurate in both directions: a widget that is genuinely
// always readonly re-reads `true` forever and nothing changes, while a field unlocked by an Edit toggle
// stops being written off. The asymmetry decides it — a stale `true` deletes coverage permanently, a
// fresh `false` costs at most one honest probe that records NOT_FILLABLE.
//
// ADDITIVE, NEVER IDENTITY — unchanged by any of this. Identity is minted in ids.mjs from
// templateSelector + instanceKey alone; nothing here is an input to it, and the churn is PROVEN rather
// than asserted (tests/unit/field-state.test.mjs runs identity-diff across a snapshot where the flag
// flips). Transient DOM readings leaking into the identity path is the 148-phantom-template failure mode
// (INC.4) and the reason that proof is the load-bearing assertion of this change, not the flag itself.
export const FIELD_STATE_KEYS = new Set(['disabled', 'readOnly']);

// How many readings of one state key are kept. Each entry is ONE SNAPSHOT's aggregate — a boolean when the
// snapshot's instances agree, `'mixed'` when they disagree — and consecutive duplicates collapse, so this
// is an ALTERNATION record over TIME: [true] never moved, [true,false] means the precondition was met,
// ['mixed'] means the instances never agreed. Capped because a control that flaps on every snapshot must
// not grow the node without bound; past the cap the list stops growing, `flips` keeps counting and
// `truncated` says so — an incomplete record, never a wrong one (the same discipline as ORIGINS_CAP). The
// CURRENT scalar always lives in `fieldFacts` (last-wins); this is the temporal record beside it.
export const FIELD_STATE_HISTORY_CAP = 8;

// Record ONE SNAPSHOT's aggregate reading of a STATE key. Called once per template per snapshot from
// mergeSnapshot (guarded by recordedState), with `value` = the snapshot aggregate (boolean or 'mixed', see
// stateAggValue) — NOT once per instance, which is what turned a multi-instance disagreement into a phantom
// same-instant flip.
//
// THE CURRENT SCALAR WINS in `fieldFacts` (set last-wins by the caller); the temporal record it replaced
// survives here, so a disabled→enabled transition is recoverable from the graph rather than silently
// overwritten. `prevValue` SEEDS the history for a node written before this record existed: a graph carried
// across runs holds `disabled:true` and no history, and without the seed the first post-upgrade reading
// would look like the first reading ever and the transition it completes would be lost.
function recordFieldState(node, key, value, prevValue) {
  const hist = node.fieldStateHistory || (node.fieldStateHistory = {});
  let rec = hist[key];
  if (!rec) {
    rec = hist[key] = { values: [], flips: 0, truncated: false };
    if (prevValue === true || prevValue === false) rec.values.push(prevValue);
  }
  const last = rec.values.length ? rec.values[rec.values.length - 1] : undefined;
  if (last === undefined) rec.values.push(value);
  else if (last !== value) {
    rec.flips += 1;
    if (rec.values.length < FIELD_STATE_HISTORY_CAP) rec.values.push(value); else rec.truncated = true;
  }
  return value;
}

// HAS THE PRECONDITION BEEN MET? True when `key` was observed `true` at some point and the CURRENT
// reading is `false` — the disabled→enabled transition OVER TIME, i.e. the crawler learned what unlocks the
// control.
//
// A TEMPORAL FLIP, NOT A SPATIAL DISAGREEMENT. The history now records ONE per-snapshot AGGREGATE per key
// (see recordFieldState / mergeSnapshot): instances that agree collapse to their shared boolean, instances
// that disagree collapse to 'mixed'. A permissions table (the current user's row disabled, everyone else's
// enabled) reads 'mixed' every snapshot — never a clean `true`, never a clean `false` — so it can neither
// satisfy `.includes(true)` nor read as enabled-now, and `fieldStateCleared` correctly stays false. Without
// this, per-instance recording wrote history `[true, false]` in ONE instant and this predicate reported an
// unlock that never happened, suppressing the genuinely-disabled rows' re-probing and their not-operable
// finding.
//
// This is the fact the RETIREMENT logic needs. `knowledge.ELEMENT_TERMINAL_BLOCKS` treats a `DISABLED`
// probe row as an answer ABOUT THE ELEMENT ("re-asking cannot change it"), which is true of a policy
// refusal and false of a state: once this returns true, that row describes a state the element has since
// LEFT and must stop discharging its obligations. graph-store deliberately does NOT act on it — retirement
// is knowledge.mjs's decision and stays there; this is the evidence it reads.
export function fieldStateCleared(node, key = 'disabled') {
  const rec = node && node.fieldStateHistory && node.fieldStateHistory[key];
  if (!rec || !rec.values.includes(true)) return false;               // never observed CLEANLY disabled
  if (rec.values[rec.values.length - 1] === 'mixed') return false;    // the latest snapshot DISAGREED — spatial, not temporal
  return !!node.fieldFacts && node.fieldFacts[key] === false;         // enabled NOW (the live scalar)
}
...
export function mergeSnapshot(graph, route, elements, opts = {}) {
  ...
  const filled = [];
  // PER-SNAPSHOT STATE AGGREGATE. One snapshot carries every instance of a template as its OWN element, so
  // recording a state key (disabled/readOnly) per instance turned a MULTI-INSTANCE DISAGREEMENT — a
  // permissions table with the current user's row disabled and the others enabled — into a within-snapshot
  // `[true,false]` flip in the template's history, which fieldStateCleared then misread as a temporal
  // unlock. Pre-scan the snapshot once per template + state key: instances that AGREE collapse to their
  // shared boolean, instances that DISAGREE collapse to 'mixed'. Each template then records its state
  // history exactly ONCE this snapshot (guarded by recordedState below). A boolean feeds the temporal
  // history; 'mixed' is a spatial disagreement fieldStateCleared refuses to read as a flip.
  const stateAgg = new Map();                 // tid -> { [key]: Set<boolean> }
  for (const el of elements) {
    if (!el.fieldFacts) continue;
    for (const k of FIELD_STATE_KEYS) {
      const v = el.fieldFacts[k];
      if (v !== true && v !== false) continue;
      let m = stateAgg.get(el.templateId);
      if (!m) stateAgg.set(el.templateId, (m = {}));
      (m[k] || (m[k] = new Set())).add(v);
    }
  }
  const stateAggValue = (tid, k) => {
    const s = stateAgg.get(tid) && stateAgg.get(tid)[k];
    if (!s || s.size === 0) return undefined;
    return s.size > 1 ? 'mixed' : s.values().next().value;
  };
  const recordedState = new Set();            // `${tid}:${key}` already recorded THIS snapshot
  for (const el of elements) {
    ...
    // FIELD FACTS — what this field DECLARES it accepts (maxLength, required, pattern, the label, the
    // hint) and what STATE it is currently in (disabled, readOnly). Stored on the template because a
    // constraint is a property of the control, not of one row's instance. Declarations merge
    // write-once-then-fill: an early snapshot may catch a field while its wrapper has not rendered its
    // `required` marker yet, so a later observation may ADD facts, but an existing non-null declaration is
    // never overwritten — the first honest reading stands. STATE is re-read on EVERY snapshot instead,
    // with the reading it replaced kept in node.fieldStateHistory. See FIELD_STATE_KEYS for the split, the
    // measurement that forced it, and why `readOnly` is on the state side.
    //
    ...
    if (el.fieldFacts) {
      ...
      for (const [k, v] of Object.entries(el.fieldFacts)) {
        ...
        // TEMPLATE-LEVEL RESOLUTION, stated because it is a real limit rather than an oversight: facts live
        // on the template, so when one snapshot carries several instances the LAST instance's reading is
        // what lands in `fieldFacts` (the current scalar). For a STATE key the temporal HISTORY is recorded
        // separately and ONCE per snapshot, from the snapshot AGGREGATE (stateAggValue) rather than per
        // instance — so a list whose row 1 is disabled and row 2 is not records 'mixed', an honest spatial
        // disagreement, instead of a phantom temporal flip. The per-instance truth stays in the probe rows,
        // which frontier already scopes per instance (rowsForInstance).
        if (FIELD_STATE_KEYS.has(k)) {
          merged[k] = v;                                    // current scalar, last-wins (unchanged)
          const sk = `${tid}:${k}`;
          if (!recordedState.has(sk)) {
            recordedState.add(sk);
            const agg = stateAggValue(tid, k);
            if (agg !== undefined) recordFieldState(node, k, agg, prev[k]);
          }
          continue;
        }
...
export function addTrigger(graph, templateId, request) {
  ...
  // The HOSTS this path-shaped pattern was actually seen on. The key stays (method, urlPattern) —
  // origin is additive metadata, never part of the identity — so a request node CAN legitimately
  // span two origins (an app and its api subdomain sharing a path); record them all rather than
  // let the first one imply the others. Capped: a wildcard-subdomain CDN must not grow the node
  // unbounded, and past the cap the field degrades to an incomplete list, never a wrong one.
  if (request.origin) {
    node.origins = node.origins || [];
    if (!node.origins.includes(request.origin) && node.origins.length < ORIGINS_CAP) node.origins.push(request.origin);
  }
