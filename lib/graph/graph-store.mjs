...
function fillRevealIfHidden(node, inst, el, route, revealPath, stateful, preVisible) {
  ...
  // replay (the live rawcaster in-app-state gap). `preVisible` is the set of controls visible IMMEDIATELY
...
export function mergeSnapshot(graph, route, elements, opts = {}) {
  ...
  for (const el of elements) {
    ...
    if (!node.instances.some((i) => i.instanceKey === el.instanceKey)) {
      ...
      // opts.stateful rides the instance reveal too (the rawcaster nav-swap control has NO node.reveal —
...
export function loadGraph(path) {
  try {
    ...
    return {
      ...
      // Carried forward so the WRITE-HUNT create-spam budget survives a mid-crawl resume (else the counter
      // resets to 0 and the drain could commit up to 2× the budget across a restart). ADDITIVE reporting/
      // budget metadata, NEVER an identity input — no schema bump; absent on a read-only graph → undefined.
      // Carried ONLY when a FINITE number: a corrupt/tampered NaN/string/negative must never reach the budget
      // comparison (`spent >= createBudget`) and silently disable the cap (security L1) — drop it → resets to 0.
      ...(Number.isFinite(raw.huntCreates) && raw.huntCreates >= 0 ? { huntCreates: raw.huntCreates } : {}),
    };
  } catch {
    return makeGraph();
  }
