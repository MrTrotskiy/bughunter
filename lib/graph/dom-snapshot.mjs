...
function collect(widgetPopupSelector) {
  ...
  // click waits out its full timeout on an element that no longer exists. Measured live on rawcaster: 89 of
  ...
  const isStateClass = (c) => /-(?:has-success|has-error|has-warning|has-feedback|focused|open|active|selected|checked|disabled|expanded|collapsed|loading|hover|pressed)$/.test(c)
  ...
  // instanceKey falls to the open-order `#N`. Result (measured live on rawcaster): Delete/Edit/Share/Block/
  ...
  const fieldFactsOf = (el) => {
    ...
    return {
      ...
      required: el.required || el.getAttribute('aria-required') === 'true'
        || !!(item && item.classList.contains('ant-form-item-required')) || null,
  ...
  // else own text with a trailing count/badge stripped ("Become a Fan (12)" → "Become a Fan"), lower-cased.
  ...
  const shortName = (el) => {
    ...
    // never be danger-judged, so it is honestly uncaptured. Measured live on rawcaster: 16 visible
