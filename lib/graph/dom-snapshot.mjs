...
function collect(widgetPopupSelector) {
  ...
  // framework CLASSES were never covered. STRUCTURAL framework classes (`ant-tabs-tab`, `ant-modal-content`)
  // are deliberately KEPT — they are the anchor we want. A TRANSITION phase is rejected here; an
  // INTERACTION/VISIBILITY STATE (`-active`, `-hidden`, …) is rejected by isStateClass below (see the note
  // there on why `ant-dropdown-hidden`, once kept as "what the UI IS", now goes with the rest).
  ...
  // `-hidden` is a VISIBILITY state (a dropdown/popover toggled shut) and fragments exactly like the rest:
  // `.ant-dropdown` (open) and `.ant-dropdown.ant-dropdown-hidden` (shut) became TWO templates for one
  // control (measured ALIAS_COLLISION 1092/1098↔1100/1108). It was once KEPT as "what the UI IS", but the
  // crawler ALREADY records visibility in the additive, non-identity `visible` field — so keeping it in the
  // SELECTOR duplicated that fact into identity and split the control, the very thing isStateClass stops.
  // SCOPED TO THE FRAMEWORK-TOGGLED FORM (`ant`/`rc`/`mui`/`radix`/`headlessui` … `-hidden`), NOT a bare
  // `-hidden` suffix: a permanent utility/accessibility class (`visually-hidden`, `sr-hidden`, `mobile-hidden`)
  // ends in `-hidden` too, and stripping it could MERGE two co-existing distinct controls that share a
  // structural path + rowKey and differ ONLY by that class (bughunter-reviewer's spatial-merge finding — a
  // permanent class is temporally constant but NOT spatially unique, so "constant ⇒ no-op" is false). The
  // framework-prefixed match de-fragments the antd/rc dropdown without touching a utility-hidden twin.
  const isStateClass = (c) => /-(?:has-success|has-error|has-warning|has-feedback|focused|open|active|selected|checked|disabled|expanded|collapsed|loading|hover|pressed|dragged|dragging)$/.test(c)
    ...
    || /^(?:ant|rc|mui|radix|headlessui)-[\w-]*-hidden$/.test(c);
  ...
  for (const e of elements) {
    ...
    } else if (e.fieldFacts && (e.fieldFacts.kind === 'select' || e.fieldFacts.kind === 'picker') && e.fieldFacts.label) {
      // ANTD WIDGET DURABLE LOCATOR (CLASS 2). An antd Select/Picker inner <input> carries no testid, its
      // `rc_*` id is framework-noise (rejected above), and its role/name are empty — so it fell to the
      // POSITIONAL css selector and went NO_INSTANCE the instant antd re-mounted the widget's internals
      // between reveal and act. The DURABLE handle is the widget's own CLICKABLE affordance
      // (`.ant-select-selector` / the `.ant-picker` box) scoped by the form-item LABEL text — both survive a
      // re-mount. Resolving to the clickable ALSO clears the click-interception (the display
      // `span.ant-select-selection-item` overlays the inner input), and actuateSelect/actuatePicker click
      // exactly this affordance to open — so one durable locator fixes reach AND actuation. Derived from
      // `fieldFacts` (already captured); ADDITIVE, never an identity input, like every other locator branch.
      e.locator = { type: 'widget', kind: e.fieldFacts.kind, label: e.fieldFacts.label,
        click: e.fieldFacts.kind === 'picker' ? '.ant-picker' : '.ant-select-selector' };
    } else {
