...
//
// PASS for wiring: a material fraction of the unreachable-with-path instances reach, and NOT_REACHED is the
// minority. FAIL: NOT_REACHED dominates → do not wire; the breadcrumbs are not paths.
...
async function main() {
  ...
  L.push('');
  L.push(`openers ${summary.probed} · reached-exact ${summary.reachedExact} · reached-representative ${summary.reachedRepresentative} · not-reached ${summary.notReached} · hop-failed ${summary.hopFailed} · refused ${summary.refused}`);
  L.push(`instances recoverable: ${summary.instancesRecoverable}/${summary.candidateInstances}`);
