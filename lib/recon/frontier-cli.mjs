...
export function emit(opts = {}) {
  ...
  // Debug trail: record what the agent is ABOUT to study this step (the receptive field)
  // + the honest denominator, so the admin timeline can show the walk's decision points.
  const runId = process.env.BUGHUNTER_RUN_ID;
  if (runId) {
    traceEvent(runId, 'frontier.emit', {
      candidates: batch.map((t) => ({ templateId: t.templateId, name: t.name, role: t.role, route: t.route })),
      stats,
    });
  }
  return { ok: true, batch, stats };
}
