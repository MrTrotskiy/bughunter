...
import { nextBatch, navBatch, frontierStats, frontierInstanceStats, RECEPTIVE_FIELD } from './frontier.mjs';
import { routeFrontierStats } from './route-frontier.mjs';
import { traceEvent, readFrontierProgress, readFrontierChurn } from '../debug/trace.mjs';
...
export function emit(opts = {}) {
  ...
  // MENU-EVENT SWEEP: lead with nav-landmark controls (constant-URL onClick sections) so they hydrate
  // first; only when none remain does the general frontier drain. navBatch purely reorders what nextBatch
  // would emit anyway (same drain predicate), so the denominator + progress signals are unchanged.
  // GOAL 5 variance source: BUGHUNTER_SEED re-permutes the emission order (deterministically) so ≥2
  // budget-capped re-crawls explore different subsets — the run-to-run difference Chao2 needs. Absent →
  // ascending order (byte-identical). Reorders emission ONLY; the drain predicate + denominator are unchanged.
  const seed = process.env.BUGHUNTER_SEED || opts.seed || undefined;
  const navFirst = navBatch(graph, { size, seed });
  const batch = navFirst.length ? navFirst : nextBatch(graph, { size, seed });
  const stats = frontierStats(graph);
  const instanceStats = frontierInstanceStats(graph);
  // Route-frontier queue (INC.1b): pages the BFS harvest discovered but has not yet snapshot-visited.
  // Surfaced so the /recon driver + decideProgress can drain queued ROUTES (route-cli --visit-next)
  // after the TEMPLATE frontier empties, instead of falsely declaring the crawl drained.
  const routeStats = routeFrontierStats(graph);
  const pendingRoutes = routeStats.pending;
  ...
  // Churn history for the typed-terminal release gate (blocker-6 Part B): the PRIOR windows' churnSkipped,
  // read off the SAME frontier.emit trail (instanceStats.churnSkipped rides in it), dropping the current
  // window — mirrors progressHistory exactly. With no runId the history is empty (churnSkipped defaults to 0
  // through the whole chain), so a runId-less caller behaves exactly as before.
  const churnHistory = runId ? readFrontierChurn(runId).slice(0, -1) : [];
  const verdict = decideProgress({
    ...
    progressHistory,
    pendingRoutes,
    churnSkipped: instanceStats.churnSkipped,
    churnHistory,
  });
  return { ok: true, batch, stats, instanceStats, routeFrontierStats: routeStats, pendingRoutes, churnSkipped: instanceStats.churnSkipped, progress: verdict };
}
