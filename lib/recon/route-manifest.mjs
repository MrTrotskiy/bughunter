...
import { toUrlPattern, isParamPath } from '../graph/graph-store.mjs';
...
// the live target denominator (measured) without ever rejecting a legal path (which uses none of them).
...
export async function extractRoutes(page) {
  ...
  for (const c of candidates) {
    ...
    // `$id` is a parameter slot exactly like `:id` — see isParamPath. Classifying it static
    // seeded it as a literal route that no server ever serves.
    if (isParamPath(c)) paramRoutes.add(c); else routes.add(c);
  }
