...
export async function resolveWithAttempts(page, inst, node) {
  ...
  try {
    ...
    // ANTD WIDGET — the durable clickable affordance (`.ant-select-selector` / `.ant-picker`) scoped by the
    // form-item LABEL text. Reached when the inner input's positional selector went stale after a re-mount.
    // The handle IS the control's own clickable affordance — actuateSelect/actuatePicker click it to open,
    // and actStep clicks it for a plain act — so it is NOT a stand-in for a different element (like the label
    // branch, not the role-name/text representatives). The label text survives a re-mount; the scope is the
    // `.ant-form-item` that contains that exact label. Ambiguous (label matched >1 form-item) → the first
    // visible is a representative, recorded truthfully so actStep re-checks its live identity before firing.
    if (loc && loc.type === 'widget' && loc.label) {
      const a = at('widget');
      a.ran = true;
      const scope = page.locator('.ant-form-item').filter({ has: page.getByText(loc.label, { exact: true }) });
      const hs = await scope.locator(loc.click || '.ant-select-selector').elementHandles().catch(() => []);
      a.raw = hs.length;
      const vis = await visibleOf(hs);
      a.visible = vis.length;
      a.sameTemplate = 0;
      // STRUCTURAL CONFIRMATION, the same discipline as the role-name/text guards one level down: a label is
      // not identity. A generic label ("Status", "Date") can sit on a select in a DIFFERENT visible form, so
      // "the one .ant-select-selector under a form-item with this exact label" is only THIS control when its
      // widget genuinely contains the recorded template element. Confirmed → exact (representative:false).
      // Unconfirmed (the widget re-mounted so the positional template path no longer matches, or the label is
      // shared) → a live REPRESENTATIVE, recorded truthfully so actStep re-checks the live identity before
      // firing — never a silent cross-control act (the same-name class that hid a wrong-control bug for runs).
      const tsel = node && node.templateSelector;
      const inWidget = async (h) => {
        if (!tsel) return false;
        return h.evaluate((el, s) => {
          try { const w = el.closest('.ant-select, .ant-picker, .ant-cascader'); return !!(w && w.querySelector(s)); }
          catch { return false; }
        }, tsel).catch(() => false);
      };
      for (const h of vis) if (await inWidget(h)) { a.sameTemplate += 1; return done(h, 'widget', false); }
      if (vis.length >= 1) return done(vis[0], 'widget', true);
    }
