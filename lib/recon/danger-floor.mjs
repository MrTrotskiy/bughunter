...
// predicate because in WRITE-HUNT mode it has a DIFFERENT relax condition than an ordinary destructive act:
...
// step.mjs gates it on a run-created-account signal, NOT on the ownsTarget item marker (an account page has
// no post-card marker). A plain "Delete post" is destructive-but-NOT-account-deletion (gated by ownsTarget).
...
// OWNERSHIP-REQUIRED — verbs that MODIFY or DESTROY an EXISTING item. In write-hunt mode these fire ONLY
// after an ownership proof (the HUNT-<runId> marker in the target's DOM — hunt-gate.ownsTarget), so the
// crawl edits/deletes ONLY content IT created, NEVER others'. Create/add/post/compose/comment/like are
// ADDITIVE (new content or a reaction) and do NOT need ownership — they never destroy another user's data.
// Fail-safe by design: an ambiguous "save" is treated as ownership-required (a mis-create named "Save" is
// over-refused, never an unowned mutation slipping through). Additive to the classifier; step.mjs consumes it.
