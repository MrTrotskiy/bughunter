...
// states needs one visit per state, and a rich page (the first target's /form-a, /dashboard) has more than
...
// Why a candidate did NOT resolve, read off the SAME per-strategy attempt record actStep uses at act
// time (resolveWithAttempts costs no extra DOM work over resolveHandle — the counts are read off the
// handle lists the strategies already built). This replaces the constant `no-live-handle` the pick event
// wrote for EVERY reject, which could not tell "the element is gone" from "it is present but hidden" from
// "the resolver found candidates and the structural guard rejected them" — three different diagnoses the
// run-log audit named as one blind bucket.
function resolveReason(attempts) {
  const ran = (attempts || []).filter((a) => a && a.ran);
  if (!ran.length) return 'not-probed';
  if (ran.some((a) => a.raw > 0 && a.visible > 0 && a.sameTemplate === 0)) return 'wrong-template'; // found, structurally rejected
  if (ran.some((a) => a.raw > 0 && a.visible === 0)) return 'present-not-visible';                   // in DOM, hidden
  return 'gone-from-dom';                                                                            // no match anywhere
}
async function resolvesLive(page, target, node) {
  const r = await resolveWithAttempts(page, target.instance, node);
  return r.handle ? { ok: true, reason: null } : { ok: false, reason: resolveReason(r.attempts) };
}
...
export async function statefulLoop(graph, { page, origin, ledger, step, budget = {}, onStep, runId, marker = null, runCreatedAccount = false, pinnedRoute = null, relogin = null } = {}) {
  ...
  const pickLive = async (cur) => {
    ...
    for (const t of ordered) {
      ...
      const live = await resolvesLive(page, t, node);
      if (live.ok) {
        ...
        if (runId) {
          ...
          traceEvent(runId, 'pick', {
            ...
            rejected: rejected.slice(0, PICK_SAMPLE).map((x) => ({
              // IDENTIFY the rejected control from the TRAIL ALONE. A pick-stage reject never reaches
              // gate/act.failed (which carry instanceSelector/authored), so templateId+empty-name left the
              // operator unable to tell WHAT template 15 was without opening the gzip graph snapshot. The
              // instanceKey (e.g. "data-id:self-1") + the durable selector (e.g. "button.rpcdel") name it.
              templateId: x.templateId, name: (x.name || '').slice(0, 40), why: x.reason || 'no-live-handle',
              instanceKey: x.instanceKey ?? null, selector: x.instance?.instanceSelector ?? null,
            })),
      ...
      rejected.push({ templateId: t.templateId, instance: t.instance, node, name: t.name, instanceKey: keyOf(t), reason: live.reason });
    }
  ...
  const recoverGated = async (cur, budget = REOPEN_BUDGET) => {
    ...
    for (let i = 0; i < budget && steps.length < maxActs; i++) {
      ...
      for (const t of candidates(graph)) {
        ...
        if ((await resolvesLive(page, t, node)).ok) continue;
        target = t; break;
      }
      if (!target) break;
      // CAPTURE THE THROW, do not discard it. `.catch(() => null)` ate the exception, so a reopen that
      // THREW (gotoGated aborted a navigation, a page detached mid-walk) recorded `code: REOPEN_THREW`
      // with NO message and NO failedHop — measured, 9 of 20 reopen failures on one run carried ZERO
      // evidence, which the project's log rule treats as a defect in the trail itself. `re` stays null on
      // a throw exactly as before, so control flow is unchanged; only the trail gains the reason.
      let re = null;
      let reThrew = null;
      try { re = await reopenContainer(page, graph, target, { origin, marker, runCreatedAccount }); }
      catch (e) { reThrew = String(e && e.message || e).slice(0, 200); }
      ...
      if (runId) {
        traceEvent(runId, 'reopen', {
          ...
          // THE THROWN EXCEPTION, no longer swallowed by `.catch(() => null)`. Null on the ordinary
          // structured-failure path (the code/failedHop above already explain it); the reopen's own
          // message when it threw — the evidence a REOPEN_THREW row used to carry none of.
          error: reThrew,
        });
      }
      ...
      let outcome = null;
      // STAMP THE REVEAL-REPLAY OUTCOME onto the target so a recovery act that FAILS carries it inline on its
      // act.failed row (L1). We are past `if (!(re && re.ok)) continue`, so the container reopened and the
      // target resolved AT reopen; if the act below still fails, the failure row then says "reopened ok, acted
      // anyway, still failed" rather than looking identical to a live-state miss. `re` also carries the rung
      // (in-place vs reload-replay) and hop count. This is the inline half of the reopen/reopen-delivered
      // events the failure-row consumer (readActFailed) never joined.
      target.revealReplay = {
        replayed: true, ok: true, code: re.code || null, rung: re.rung || null, hops: re.attempt ?? null,
      };
      try {
  ...
  while (true) {
    ...
    // having never queued /groups, /events, /items or /setting. Consulting it FIRST meant the driver
  ...
  // TERMINAL EVENT — the ONE line that says WHY the crawl stopped, on the trail itself. Before this the
  // verdict lived only in run.json; an operator reading events.ndjson had to reconstruct "frontier-drained"
  // from the tail (drain-outcome + retire final:true + route-choice eligible:0), which is exactly the
  // "reconstruct the conclusion from four rows" defect the run-log audit named. `reason` is the same value
  // the caller closes run.json with, and the frontier stats say what was and was NOT reached at the stop.
  const terminalReason = stopped || 'frontier-drained';
  const finalStats = frontierStats(graph);
  if (runId) traceEvent(runId, 'loop-terminal', { reason: terminalReason, steps: steps.length, stats: finalStats });
  return { steps, stopped: terminalReason, stats: finalStats, reloc: relocMemo.stats() };
}
