  ...
  return async (route) => {
    ...
    try {
      ...
      if (danger || offOrigin) { // (1) HARD unless it is a SAFE-method OR benign-telemetry off-origin sub-resource
        blocked.push({ method, urlPattern, reason: danger ? 'danger-route' : 'off-origin', hard: danger || (!safe && !BENIGN_TELEMETRY.test(urlPattern)) });
        await route.abort();
        return;
      }
      ...
      blocked.push({ method, urlPattern, reason: tele ? 'telemetry' : 'write', hard: !tele }); // L1: pattern only, never the raw url
      await route.abort();
    } catch {
