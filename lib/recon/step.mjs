...
// shared with the recon-run navigation guard). The coarse floor is a backstop, not the
// judge — but it must guard the ACT, not just the post-hoc record. Refusing before the
// click means an obvious delete/logout/checkout is never fired, whatever path reached
// actStep (whats-new --act-template, the loop, a mis-judging agent). The deliberate-
// mutation phase (firing your OWN HUNT-tagged control) is a future scoped override.
