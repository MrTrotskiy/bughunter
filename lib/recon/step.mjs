...
import { ownsTarget, ownsViaReveal, inOwnableItem } from './hunt-gate.mjs';
...
export async function actStep(page, graph, ledger, target, { fill, prefill, capture, revealPath, openerReplayable, refuseMutations, revealOpener, statefulProvenance, huntWrites, marker, runCreatedAccount, huntWindow, huntStrict } = {}) {
  ...
  // NODE-LOOP STRICTNESS (security H1; decisions.md 2026-07-18 "NODE-LOOP HUNT"): the JUDGE-FREE node-loop
  // passes huntStrict. A benign-METHOD + benign-PATH mutation (a GraphQL/RPC `POST /graphql {deletePost}`)
  // slips BOTH the name gate (nameless → requiresOwnership false) AND the firewall's destructive method/path
  // gate, so an UNOWNED destructive write on ANOTHER user's content would commit — the agent path had the LLM
  // as a de-facto backstop, the node-loop has none. So in strict mode ONLY arm the write window when ownership
  // is PROVEN or the control is NOT inside an ownable item (a create composer — the SOLE safe unowned write).
  // This also declines like/comment on OTHERS' items on the node-loop (their endpoint is still collected via
  // OWN content); the agent path (no huntStrict) keeps its judged behavior. Fail-closed (inOwnableItem → true
  // on error → decline). When declined, the window stays shut so the firewall aborts the write read-only.
  let huntArmed = huntWrites;
  if (huntArmed && huntStrict && !huntOwned) huntArmed = !(await inOwnableItem(handle));
  ...
  try {
    ...
    if (huntWindow && huntArmed) { huntWindow.on = true; huntWindow.owned = huntOwned; }
