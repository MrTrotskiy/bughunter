...
import { dangerFloor, mutationFloor, REFUSED, routeRefused } from './danger-floor.mjs';
...
export async function actStep(page, graph, ledger, target, { fill, capture, revealPath, openerReplayable, refuseMutations, revealOpener, statefulProvenance } = {}) {
  ...
  if (refuseMutations && !revealOpener && mutationFloor({ name: gateName, route: gateRoute }) === 'mutation') {
    throw envelopeError({
      code: 'MUTATION_FLOOR',
  ...
  if (fill != null && fill !== '') await fillTarget(handle, String(fill));
