...
export async function actStep(page, graph, ledger, target, { fill, capture, revealPath } = {}) {
  ...
  if (sameOrigin(startUrl, landedUrl)) {
    // GAP 2: stamp reveal only on a stay-on-page act with all-GET caused requests (a mutating opener's children stay unreachable).
    const stamp = revealPath && landedRoute === route && requests.every((r) => String(r.method || 'GET').toUpperCase() === 'GET');
    route = landedRoute;
