...
import { actuateAll } from './field-actuate.mjs';
...
export async function actStep(page, graph, ledger, target, { fill, prefill, capture, revealPath, openerReplayable, statefulProvenance, marker, runCreatedAccount, exploreAll, stateDir, runId } = {}) {
  ...
  if (fill != null && fill !== '') {
    ...
    await fillTarget(handle, stampOwnership({ value: fill }, marker).value);
  }
  ...
  if (sameOrigin(startUrl, landedUrl)) {
    ...
    // deliberately. Measured cost of leaving it on: on rawcaster's POST-read /dashboard only 22% of
