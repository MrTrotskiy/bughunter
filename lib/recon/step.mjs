...
import { mergeSnapshot, addTrigger, toUrlPattern, markOpener } from '../graph/graph-store.mjs';
import { beginCause, endCause, waitSettled, resetCause } from '../browser/causal.mjs';
import { dangerFloor, REFUSED } from './danger-floor.mjs';
...
export async function actStep(page, graph, ledger, target, { fill, capture, revealPath, openerReplayable } = {}) {
  ...
  if (sameOrigin(startUrl, landedUrl)) {
    ...
    const stamp = revealPath && landedRoute === route && (allGet || openerReplayable === true);
    route = landedRoute;
    const afterMerge = mergeSnapshot(graph, landedRoute, after.elements, stamp ? { revealPath } : {});
    newElements = afterMerge.newInstances.map((i) => ({ templateId: i.templateId, instanceKey: i.instanceKey }));
    // OPENER detection (state model): this control revealed new instances WITHOUT leaving the route
    // — a stay-on-page opener. Flag its template so the frontier walks its OTHER instances too (a nav
    // bar of 3 links that are instances of one template gets all three explored, not just the first).
    if (newElements.length > 0 && landedRoute === routeKey(startUrl)) markOpener(graph, tid);
  }
