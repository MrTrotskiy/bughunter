...
import { writeSkeleton, traceEvent } from '../debug/trace.mjs';
...
// THE DECISION RECORD (`gate` event). The operator's standing requirement is to know why the crawler
// PRESSED, why it did NOT, and at which rule — for EVERY offered control, in BOTH modes. Before this,
// a refusal survived only as a relabelled `act.failed` (an OUTCOME, not the REASON), and a PERMIT — the
// positive "I decided to fire this, here is the classification and the ownership cost" — had no event at
// all. `gate` is that reason, distinct in `kind` from the act it precedes: `decision` is permit|refuse,
// `stage` names the rule that decided (danger-floor / account-precheck / live-representative / href-route /
// ownership-rail / cleared), and it carries the danger `floor`, the `authored` id the name-blind gate now
// reads, the live `ownership` verdict and `resolveMs` — the DOM ownership-proof cost the plan measured at
// ~30% of wall clock, the single largest non-act time sink. Emitted ONLY in the pre-`beginCause` gate
// stack, so it is a pure disk append that cannot perturb causal attribution (step.mjs line 134). runId is
// optional: no run, no trail, no gate row — never a throw on the act path.
function emitGate(runId, fields) {
  if (!runId) return;
  try { traceEvent(runId, 'gate', fields); } catch { /* a decision record must never break the decision */ }
}
...
export async function actStep(page, graph, ledger, target, { fill, prefill, capture, revealPath, openerReplayable, statefulProvenance, marker, runCreatedAccount, exploreAll, stateDir, runId } = {}) {
  ...
  // The fields shared by every `gate` row this act may emit — the WHAT (which control, how named) and the
  // MODE. Each decision site spreads this and adds its own `decision`/`stage`/`code`. `ownershipVerdict`
  // is filled by the explore-all rail below and reported on the eventual permit; `resolveMs` times the
  // durable resolution + the live ownership probes (the ~30% wall-clock cost the plan flagged).
  const gateBase = { templateId: tid, name: gateName ?? null, authored: gateAuthored ?? null, route: gateRoute ?? null, floor, mode: exploreAll ? 'explore-all' : 'default' };
  let ownershipVerdict = null;
  let ownershipVia = null; // WHICH ownership check decided (marker-on-handle / any-hunt / reveal-trigger / not-in-item / no-marker-in-item) — the BASIS the gate event records, so a same-post own-vs-foreign flip is explainable, not a black box
  if (exploreAll) {
    ...
    if (!pre.allow) {
      emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'account-precheck', code: pre.code });
      throw envelopeError({ code: pre.code, message: `${pre.reason} — "${gateName ?? sel}" (template ${tid})`, exit: 'VIOLATION' });
    }
  } else if (REFUSED.has(floor)) {
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'danger-floor', code: 'DANGER_FLOOR' });
    throw envelopeError({
      code: 'DANGER_FLOOR',
  ...
  // graph mutation.
  const resolveStart = Date.now(); // the ownership-proof cost the permit gate reports (~30% wall clock)
  ...
  if (!resolved.handle) {
    ...
    if (storedAttempt && storedAttempt.raw > 0) {
      emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'resolve', code: 'NOT_VISIBLE', resolveMs: Date.now() - resolveStart });
    ...
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'resolve', code: 'NO_INSTANCE', resolveMs: Date.now() - resolveStart });
  ...
  if (representative) {
    ...
    if (exploreAll) {
      ...
      if (!liveVerdict.allow) {
        emitGate(runId, { ...gateBase, name: liveN ?? null, decision: 'refuse', stage: 'live-representative', code: liveVerdict.code, resolveMs: Date.now() - resolveStart });
      ...
      if (REFUSED.has(liveFloor)) {
        emitGate(runId, { ...gateBase, name: (liveName || node?.name || target.name) ?? null, decision: 'refuse', stage: 'live-representative', floor: liveFloor, code: 'DANGER_FLOOR', resolveMs: Date.now() - resolveStart });
        throw envelopeError({
          code: 'DANGER_FLOOR',
  ...
  if (href && isOffOriginHttp(startUrl, href)) {
    ...
    // ORIGIN, not the raw href (decisions.md 2026-07-20 [L1]): a secret can ride an outbound SSO/share
    // link's query, and the trail is served by admin-server. The origin keeps the diagnostic "which
    // external site" and drops the query — the sibling href-route gate below strips it the same way.
    const offOrigin = (() => { try { return new URL(href).origin; } catch { return null; } })();
    emitGate(runId, { ...gateBase, decision: 'skip', stage: 'off-origin', code: 'OFF_ORIGIN', href: offOrigin, resolveMs: Date.now() - resolveStart });
  ...
  if (href && !exploreAll && routeRefused(routeKey(href))) {
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'href-route', code: 'DANGER_FLOOR', href: routeKey(href), resolveMs: Date.now() - resolveStart });
  ...
  if (exploreAll) {
    let ownership;
    // Record WHICH check decided, not just the verdict — a same-post Edit=own vs Delete=foreign flip was a
    // black box (the outcome logged, the basis lost). Each branch stamps ownershipVia so the gate says WHY.
    if (await ownsTarget(handle, marker)) { ownership = OWNERSHIP.OWN; ownershipVia = 'marker-on-handle'; }
    else if (await ownsAnyHunt(handle)) { ownership = OWNERSHIP.OWN; ownershipVia = 'any-hunt-marker'; }
    else if (revealPath && revealPath.length && marker && await ownsViaReveal(page, graph, revealPath, marker)) { ownership = OWNERSHIP.OWN; ownershipVia = 'reveal-trigger-marker'; }
    else if (!(await inOwnableItem(handle))) { ownership = OWNERSHIP.NONE; ownershipVia = 'not-in-ownable-item'; }
    else { ownership = OWNERSHIP.FOREIGN; ownershipVia = 'no-marker-in-item'; }

    ownershipVerdict = ownership;
    const verdict = decidePolicy({ name: gateName, route: gateRoute, ownership, runCreatedAccount });
    if (!verdict.allow) {
      emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'ownership-rail', code: verdict.code, ownership, ownershipVia, resolveMs: Date.now() - resolveStart });
  ...
  if (!visible) {
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'visibility-recheck', code: 'NOT_VISIBLE', resolveMs: Date.now() - resolveStart });
  ...
  if (!enabled) {
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'disabled', code: 'DISABLED', resolveMs: Date.now() - resolveStart });
    throw envelopeError({ code: 'DISABLED', message: `instance ${sel} is visible but disabled — it cannot be operated in this state` });
  }
  ...
  if (!claim.ok) {
    emitGate(runId, { ...gateBase, decision: 'refuse', stage: 'alias', code: 'ALIAS_COLLISION', resolveMs: Date.now() - resolveStart });
    throw envelopeError({
  ...
  // PERMIT — every gate above cleared, the click is about to fire. This is the positive decision record
  // that had no event before: the operator can now read the `gate` stream ALONE and see, for every offered
  // control, whether it was fired and — via `ownership`/`floor`/`resolveMs` — on what basis and at what cost.
  // `ownership` is the live explore-all verdict (null in default mode, where the danger-floor decided).
  emitGate(runId, { ...gateBase, decision: 'permit', stage: 'cleared', ownership: ownershipVerdict, ownershipVia, via, representative: representative === true, resolveMs: Date.now() - resolveStart });
