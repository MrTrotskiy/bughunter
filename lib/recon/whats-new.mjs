...
export async function run(opts) {
  ...
  // Single source of truth for the write policy (read-only-firewall.resolveWritePolicy). readOnly is the
  // abort-by-default gate; huntWrites (a STRICT SUBSET) arms the SAFE own-content mutation window when the
  // operator opts in (BUGHUNTER_HUNT / --hunt) on an authed run. Off by default → byte-identical read-only.
  const { readOnly, huntWrites } = resolveWritePolicy(process.env, { readOnly: opts.readOnly, hunt: opts.hunt, runId: opts.runId });
  // The per-run HUNT-<runId> ownership marker (only in hunt mode); runCreatedAccount gates account-deletion
  // (the operator's rule: delete an account only if THIS run created it — never a persistent test account).
  const marker = huntWrites ? huntMarker(opts.runId || process.env.BUGHUNTER_RUN_ID) : null;
  ...
  try {
    ...
    if (opts.actTemplate != null) {
      ...
      // H1 (security review): a reveal-opener lifts the NAME gate off a mutation-named control, but the
      // write-firewall nets non-GET only — so an obvious mutation committed over a side-effectful GET
      // (RPC-over-GET, `GET /api/follow?id=`) would slip. Flip the firewall's strict-GET flag ON for the
      // MEASURED act (not the __idle__ reveal replay above) so a mutation-verb GET is aborted too; always
      // reset it in finally so the next act's benign reads are unaffected. No-op unless revealOpener.
      if (roFirewall && opts.revealOpener) roFirewall.strict.get = true;
      try {
        result.acted = await actStep(
          ...
          // WRITE-HUNT: huntWrites relaxes the refusals to SAFE own-content mutations; marker is the
          // HUNT-<runId> ownership proof, runCreatedAccount gates account-deletion, huntWindow is the
          // firewall write window actStep opens for THIS act only after the ownership/account gate passes.
          { fill: opts.fill, prefill: opts.prefill, capture: runId ? makeCapture(runId, tid) : undefined, revealPath, openerReplayable: opts.openerReplayable, refuseMutations: readOnly, revealOpener: opts.revealOpener, huntWrites, marker, runCreatedAccount, huntWindow: huntWrites && roFirewall ? roFirewall.hunt : null },
        );
      } finally {
        if (roFirewall) roFirewall.strict.get = false;
      }
...
async function main() {
  ...
  try {
    ...
    // --reveal-opener: the agent's judgment that THIS mutation-NAMED control OPENS a form/modal (a read
    // that reveals UI) rather than submitting — so the compose surface is collected while the network
    // write-firewall aborts any actual submit. Agent-set only; never for a Delete/Logout/Pay/Call (those
    // are hard-refused by dangerFloor regardless). SAFETY: sound only on HTTP-mutating targets (see M1).
    const revealOpener = args['reveal-opener'] === true || args['reveal-opener'] === 'true';
    ...
    // --hunt: WRITE-HUNT mode — relax read-only to SAFE own-content mutations (create/edit-own/delete-own/
    // comment/like/pay). Needs an authed run + BUGHUNTER_RUN_ID (the ownership marker). --created-account
    // confirms THIS run created the account (gates account-deletion; default off protects a persistent account).
    const hunt = args['hunt'] === true || args['hunt'] === 'true';
    ...
    const result = await run({
      url: args.url, actTemplate: args['act-template'], instance: args.instance, fill: args.fill, prefill: prefill.length ? prefill : undefined, openerReplayable, revealOpener,
      readOnly: args['read-only'] === true, allowBenignPost: args['allow-benign-post'] === true, hunt, runCreatedAccount,
    });
