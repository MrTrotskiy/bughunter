    ...
    if (opts.actTemplate != null) {
      ...
      try {
        result.acted = await actStep(
          ...
          { fill: opts.fill, capture: runId ? makeCapture(runId, tid) : undefined, revealPath, openerReplayable: opts.openerReplayable, refuseMutations: readOnly, revealOpener: opts.revealOpener, huntWrites, marker, runCreatedAccount, huntWindow: huntWrites && roFirewall ? roFirewall.hunt : null },
        );
      } finally {
...
async function main() {
  ...
  try {
    ...
    const result = await run({
      url: args.url, actTemplate: args['act-template'], instance: args.instance, fill: args.fill, openerReplayable, revealOpener,
