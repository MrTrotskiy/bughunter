...
import { makeCapture, traceEvent, snapshotGraph, activeRunId, openRun, driverOpen } from '../debug/trace.mjs';
...
export async function run(opts) {
  // ONE crawl per state dir, on the AGENT path too. `recon-run` has claimed the dir since the retraction
  // (state-lock.mjs documents what two crawls sharing a graph cost), but this CLI — the one the semantic
  // subagent drives — did loadGraph/saveGraph with no claim at all. A second agent-path crawl against the
  // same dir would reproduce the clobbering silently, printing rising coverage the whole time. Claimed
  // before the first read and released in a finally, so a throw anywhere below still frees the dir.
  const stateDir = process.env.BUGHUNTER_STATE_DIR || path.join(process.cwd(), 'state');
  const releaseLock = acquireStateDir(stateDir, { runId: opts.runId || process.env.BUGHUNTER_RUN_ID || null });
  try {
    return await runLocked(opts, stateDir);
  } finally {
    releaseLock();
  }
}

async function runLocked(opts, stateDir) {
  ...
  try {
    ...
    if (runId) {
      // THE DRIVER RECORD: whats-new is the AGENT path (the recon subagent calls this CLI). Idempotent —
      // driverOpen head-scans the trail, so only the first of a run's many whats-new processes records it.
      driverOpen(runId, { driver: 'agent', flags: { url: opts.url, actTemplate: opts.actTemplate ?? null, openerReplayable: !!opts.openerReplayable, authed: !!process.env.BUGHUNTER_STORAGE_STATE } });
