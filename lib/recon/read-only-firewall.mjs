...
// DESTRUCTIVE-only path verbs (a SUBSET of MUTATION_VERBS that MODIFY/DESTROY an existing resource — NOT
// the additive create/comment/like/follow/share). Used by the WRITE-HUNT gate: a destructive write commits
// ONLY on proven-own content; an additive one commits freely. Leading `\bverb` like WRITE_VERB_RE so a
// compound path segment (`/deletepost`) still matches. Kept here (the firewall's own concern) so the hunt
// method-gate does not depend on the read-only vocabulary.
const DESTRUCTIVE_VERBS = 'delete|remove|destroy|erase|discard|trash|wipe|purge|drop|edit|update|archive|unpublish|deactivate|revoke|terminate';
export const DESTRUCTIVE_PATH_RE = new RegExp(`\\b(${DESTRUCTIVE_VERBS})`, 'i');
// HTTP methods that inherently MODIFY/DESTROY an existing resource (REST semantics), so they need the
// ownership proof even when the URL path carries no verb (an icon-only DELETE on /api/posts/42).
...
export function makeReadOnlyHandler(page, origin, blocked, readAllow = new Set(), opts = {}) {
  ...
  // hunt is a LIVE flag object (like strict.get) actStep flips around a WRITE-HUNT act. {on} marks the act
  // a hunt act; {owned} marks that actStep PROVED ownership (the HUNT marker in the target's item DOM, or a
  // run-created-account deletion). The DOM ownership proof works even for an ICON-only control (no name),
  // which the name-based classifiers miss — so the DESTRUCTIVE gate below keys on the HTTP METHOD + a
  // destructive path verb, NOT the control name. A destructive write (DELETE/PUT/PATCH, or a delete/remove/
  // edit/… path) commits ONLY when owned; an ADDITIVE write (POST create/comment/like) commits under hunt.on.
  // Default OFF → a normal read-only crawl is byte-identical. Per-act scoped: reset in actStep's finally.
  const hunt = opts.hunt && typeof opts.hunt === 'object' ? opts.hunt : { on: false, owned: false };
  return async (route) => {
    ...
    try {
      ...
      if (hunt.on) {                                              // WRITE-HUNT act in progress
        // A DESTRUCTIVE write (REST DELETE/PUT/PATCH, or a delete/remove/edit/… path verb) commits ONLY when
        // actStep PROVED ownership for this act (hunt.owned — the HUNT marker in the target's item DOM, which
        // catches an ICON-only control the name classifiers miss, or a run-created-account deletion). An
        // unowned destructive write ABORTS (read-only fallback) — so the crawl can never delete/edit another
        // user's content even via an unnamed control. An ADDITIVE write (POST create/comment/like) commits.
        const destructive = DESTRUCTIVE_METHODS.has(method) || DESTRUCTIVE_PATH_RE.test(urlPattern);
        if (destructive && !hunt.owned) {
          blocked.push({ method, urlPattern, reason: 'hunt-unowned-blocked' });
          await route.abort();
          return;
        }
        blocked.push({ method, urlPattern, reason: 'hunt-write' });
...
export async function installReadOnlyFirewall(page, opts = {}) {
  ...
  // The live WRITE-HUNT window (default OFF). actStep flips hunt.on + hunt.owned around a hunt act; a
  // destructive write commits only when hunt.owned (ownership proven); a normal crawl never flips it.
  const hunt = { on: false, owned: false };
  const handler = makeReadOnlyHandler(page, origin, blocked, readAllow, { allowBenignPost: !!opts.allowBenignPost, strict, hunt });
  await page.route('**/*', handler);
  const teardown = async () => { await page.unroute('**/*', handler).catch(() => {}); };
  return { blocked, readAllow, strict, hunt, teardown };
}

// Resolve the write policy from env + opts — the SINGLE source of truth both recon-run and whats-new read,
// so readOnly (and the WRITE-HUNT relaxation) is derived in ONE place, not duplicated. readOnly is the
// abort-by-default gate (an authed session OR an explicit --read-only/--stateful opt). huntWrites is a
// SUBSET of readOnly (the firewall must be installed to relax it), but it is NOT gated on AUTHENTICATION —
// `--read-only --hunt` arms it on any target. The real safety is the TRIPLE opt-in (readOnly + the explicit
// BUGHUNTER_HUNT/--hunt + a BUGHUNTER_RUN_ID for the HUNT-<runId> marker) + the OPERATOR's judgment that the
// login account is a disposable TEST account. Off by default → byte-identical to today's read-only crawl.
export function resolveWritePolicy(env = process.env, opts = {}) {
  const readOnly = !!opts.readOnly || !!opts.stateful || !!env.BUGHUNTER_STORAGE_STATE;
  const huntWrites = readOnly && (!!opts.hunt || !!env.BUGHUNTER_HUNT) && !!(opts.runId || env.BUGHUNTER_RUN_ID);
  return { readOnly, huntWrites };
}
...
export function summarizeBlocked(blocked) {
  ...
  const huntWrites = blocked.filter((b) => b.reason === 'hunt-write'); // WRITE-HUNT: safe own-content mutations COMMITTED
  const huntUnownedBlocked = blocked.filter((b) => b.reason === 'hunt-unowned-blocked'); // destructive-on-non-owned, ABORTED
  const refusedPatterns = Array.from(new Set([...writeVerb, ...writeBlocked, ...huntUnownedBlocked].map((b) => `${b.method} ${b.urlPattern}`)));
  return {
    ...
    huntWrites: huntWrites.length,
    huntUnownedBlocked: huntUnownedBlocked.length,
    huntWritePatterns: Array.from(new Set(huntWrites.map((b) => `${b.method} ${b.urlPattern}`))),
    refusedPatterns,
  };
}
