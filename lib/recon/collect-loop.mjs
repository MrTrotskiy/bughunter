...
import { outstandingOf } from './outstanding.mjs';
...
function snapshot(graphPath) {
  ...
  const knowledge = knowledgeStats(graph);
  // The work QUEUE, not the effort counter: how many elements still owe a probe, and how many answered.
  // Read here so the controller's terminal condition and its progress line speak the same language the
  // operator reads in `report --outstanding`.
  const out = outstandingOf(graph);
  return { f, fi, r, rows, endpoints, knowledge, owed: out.outstanding, studied: out.studied, studiedTotal: out.total };
}

// Fully collected ⟺ no unexplored control templates AND no pending pages in the route queue AND nothing
// still OWED.
//
// The third term is not bookkeeping pedantry, it is the difference between two claims the controller was
// conflating. `frontierStats.remaining` counts templates the frontier can still EMIT; `outstandingOf`
// counts obligations elements still owe. Those diverge exactly when an element has been touched but not
// STUDIED — which, since the probe battery landed, is the normal mid-crawl condition. Measured: a graph
// reporting remaining=0 and pending=0 while 133 obligations were owed, i.e. the controller would have
// declared the application collected with a third of its knowledge un-gathered.
//
// docs/GOAL.md: "Phase 1 is done for an application when [every element's obligation list is empty] and
// the outstanding count is zero with nothing hidden in an uncounted bucket." A terminator that never
// consults the outstanding count cannot honour that, however green everything else looks.
export const isDrained = (s) => s.f.remaining === 0 && s.r.pending === 0 && (s.owed || 0) === 0;
...
function renderLedger(round, s) {
  ...
  for (const r of s.rows) {
    ...
    let status;
    // A param PATTERN is not a failure — it is a template whose concrete instances are collected as
    // their own routes. Saying "unreachable" about it sent the reader looking for a bug that was a design.
    if (r.unreachableReason === 'param-pattern') status = '◆ param template (instances collected separately)';
    else if (r.unreachable) status = `✗ unreachable${r.unreachableReason ? ' (' + r.unreachableReason + ')' : ''}`;
  ...
  const dangers = Object.entries(t.byDanger).sort((a, b) => b[1] - a[1])
    .map(([k, v]) => `${v} ${k}`).join(' · ');
  // THE LADDER IS THE HEADLINE, because it is the metric the project is actually judged on. The line below
  // used to report `t.known` — elements carrying a purpose/danger/effect written by the `observe` CLI. The
  // stateful path never calls `observe`, so that counter is structurally ZERO here however much the run
  // learns: it printed "UNDERSTOOD: 0 classified" on a run whose ladder held 51 CHARACTERIZED elements.
  // A log that reports zero while the graph holds the opposite is worse than no log, because it is believed.
  const k = s.knowledge;
  const rungs = ['L4', 'L3', 'L2', 'L1', 'L0', 'L-1']
    .filter((r) => k.byLevel[r]).map((r) => `${r}:${k.byLevel[r]}`).join(' ');
  lines.push(`KNOWLEDGE: ${k.knowledgePct}% understood (${k.understood}/${k.obligations} at L3+)  ${rungs}`);
  // The narrower, industry-comparable denominator, shown BESIDE the honest one — never in place of it.
  if (k.declarative && k.declarative.obligations) {
    lines.push(`  of which declaratively interactive (links/buttons/fields): ${k.declarative.pct}%`
      + ` (${k.declarative.understood}/${k.declarative.obligations}) — the rest are click-handler divs`);
  }
  if (k.blocked.length) {
    // Blocked is LISTED, never a silent subtraction from the denominator.
    const by = {};
    for (const b of k.blocked) by[b.code] = (by[b.code] || 0) + 1;
    lines.push(`  BLOCKED: ${Object.entries(by).map(([c, n]) => `${n} ${c}`).join(' · ')}`);
  }
  // The semantic layer stays reported, but named for what it is, so it can never again be read as the
  // headline when it is simply not the mechanism this mode uses.
  lines.push(`  semantic notes: ${t.known} with purpose/danger/effect`
    + ` · ${t.untouched} never clicked`
...
export async function collectLoop({ url, chunk = 8, maxRounds = 40, stateful = false, log = (m) => process.stdout.write(m + '\n') }, deps = {}) {
  ...
  for (round = 1; round <= maxRounds; round++) {
    ...
    // THE NUMBER THE OPERATOR ASKED FOR, every round: how much is STUDIED and how much is still OWED.
    // "+22 controls collected" is effort; "171 still owed" is the distance to done, and only the second one
    // is allowed to decide when to stop. Printing the delta too, because an outstanding count that RISES is
    // the honest signature of newly-discovered work rather than a regression, and the operator should be
    // able to tell those apart without reading the graph.
    const dOwed = after.owed - before.owed;
    log(`     studied ${after.studied}/${after.studiedTotal} · still owed ${after.owed}${dOwed === 0 ? '' : ` (${dOwed > 0 ? '+' : ''}${dOwed})`}`);
