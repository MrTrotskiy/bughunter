...
// reported 18 "write endpoints" and the truthful count was ONE. Every one of `listitems`,
// `getprofile`, `searchusers`, `getfaq`, `listevents` is a READ that happens to travel by
...
// start) so `list`/`get`/`search` match `listitems` and `get_item_detail` but not, say, `budget`.
...
// real. `audiocontent` (text-to-speech), `itemlist`, `texttoaudio` all arrived here, and so did
