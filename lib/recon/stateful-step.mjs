  ...
  return async (graph, target) => {
    ...
    try {
      ...
      if (err && !err.envelope) {
        ...
        const reBlocked = await resolveHandle(page, target.instance, graph.elements[target.templateId]).catch(() => null);
    ...
    const probeRow = await recordProbe(graph, target, res, { page, prefill: prefillUsed, selfFill, seen: seenBefore, domBefore });
