...
// Does this raw click failure name a POINTER INTERCEPTION? Keyed on the MESSAGE SHAPE, not a hardcoded
// app class: Playwright prints the intercepting element verbatim ("<div …> intercepts pointer events"),
// so this recognises the obscured-by-a-leftover-modal timeout on ANY framework. Used to gate the app-modal
// fallback below so it fires ONLY on an interception failure, never on an ordinary click timeout.
function isInterceptionTimeout(err) {
  return /intercepts pointer events/i.test(String(err?.message || ''));
}

// Identify the leftover modal OBSCURING the blocked target (CLASS 1b). A prior act left a dialog open over
// a control on the base page — the crawler recovers reach by closing it (closeAppModal below), but the fact
// that one act's modal covered an unrelated control is itself a defect worth a human's eye (docs/GOAL.md:
// the interesting event must not vanish once reach is recovered). Returns a short descriptor of the topmost
// dialog that does NOT contain the target, or null. Read-only, under __idle__ — opens no causal window.
async function describeBlockingModal(page, blocked) {
  try {
    return await page.evaluate((blockedEl) => {
      const isVisible = (el) => {
        const r = el.getBoundingClientRect(); const st = getComputedStyle(el);
        return r.width > 0 && r.height > 0 && st.visibility !== 'hidden' && st.display !== 'none' && st.opacity !== '0';
      };
      const dialogs = [...document.querySelectorAll('[role="dialog"],[aria-modal="true"],.ant-modal,.modal,.MuiDialog-root,.ReactModal__Content')]
        .filter((d) => isVisible(d) && !(blockedEl && d.contains(blockedEl)));
      if (!dialogs.length) return null;
      const top = dialogs[dialogs.length - 1];
      const titleEl = top.querySelector('.ant-modal-title,[class*=title i],h1,h2,h3');
      const title = (top.getAttribute('aria-label') || (titleEl && titleEl.textContent) || '').trim().slice(0, 60) || null;
      const tag = (String(top.className || '').split(/\s+/).find((c) => /modal|dialog/i.test(c))) || top.getAttribute('role') || 'dialog';
      return { tag, title };
    }, blocked || null).catch(() => null);
  } catch { return null; }
}

// APP-MODAL FALLBACK (the modal-heavy-dashboard fix). dismissBlockingOverlay closes the overlays it
// recognises — Escape-honouring dialogs, curated close classes (.ant-modal-close / [aria-label=Close] /
// [data-dismiss]), and known backdrops (.ant-modal-wrap / .modal-backdrop). A modal that IGNORES Escape
// and exposes ONLY a text Cancel/Close — the AntD "Schedule a Meeting" shape that left the live Create
// opener click-timed-out — defeats every one of those, so the obscured opener stayed unreachable and the
// form behind it was refused. This closes the TOPMOST blocking modal by its own NON-COMMITTING affordance.
//
// CAUSAL DISCIPLINE, identical to dismissBlockingOverlay: a pure DOM key/click under __idle__ (the caller
// runs this only after actStep reset the cause on the throw, and resets the tracker verdicts around it), so
// it opens NO causal window and forges NO edge — a request the close happens to fire is uncredited.
// SAFE BY CONSTRUCTION: it never closes a dialog that CONTAINS the blocked target (that would destroy the
// state we are studying — but the caller only reaches here because the target is OBSCURED, i.e. outside the
// modal), and it clicks only non-committing close/cancel affordances, never an OK/Save/Submit. Returns true
// iff the blocked target became clickable again — the same handle-based verdict dismissBlockingOverlay uses.
async function closeAppModal(page, blocked) {
  try {
    // Escape first — the universal close, honoured by Radix/MUI/HeadlessUI/AntD. A no-op on the modal
    // that motivates this helper, which is why the affordance sweep below follows it.
    await page.keyboard.press('Escape').catch(() => {});
    await page.evaluate((blockedEl) => {
      const isVisible = (el) => {
      ...
      const top = dialogs[dialogs.length - 1]; // topmost by DOM order
      // A curated close affordance first (an X button), then a NON-COMMITTING text button.
      const byAttr = top.querySelector('[aria-label="Close" i],.ant-modal-close,[data-dismiss],button.close,.modal-close');
      if (byAttr && isVisible(byAttr)) { byAttr.click(); return true; }
      const RE = /^(cancel|close|dismiss|no thanks|no|back|done|✕|×|x)$/i;
      for (const b of top.querySelectorAll('button,[role="button"],a')) {
        const t = (b.innerText || b.getAttribute('aria-label') || '').trim();
        if (isVisible(b) && RE.test(t)) { b.click(); return true; }
      }
      return false;
    }, blocked || null).catch(() => {});
    // Honest verdict: "did it work" means "can the blocked target be clicked now" (the browser's hit-test).
    if (blocked) return !(await clickIntercepted(page, blocked).catch(() => true));
    return true;
  } catch {
    return false;
  }
...
export function statefulStep({ page, origin, baselineUrl, ledger, runId, graphPath, exploreAll, stateDir, marker, runCreatedAccount }) {
  ...
  const recordProbe = async (g, target, res, { page: pg, prefill, selfFill, seen, blocked, domBefore, kind }) => {
    ...
    // only 5xx of the whole crawl (`POST /app/groups -> 500`) came from an act that threw, so
    ...
    // `POST /app/sendrequest -> 200` (a friend request genuinely sent) and
    // `POST /app/additems` were both recorded as `read` because of exactly this. The classifier
  ...
  return async (graph, target) => {
    ...
    const recordFail = async (err) => {
      ...
      traceEvent(runId, 'act.failed', {
        ...
        // WHETHER REVEAL-REPLAY WAS INVOLVED — the answer to "was this a live-state miss, or did the
        // container reopen and the control STILL not act?" (L1). The stateful main pass acts on accumulated
        // live state and does NOT replay a reveal path, so its failures carry `{replayed:false}` — an honest
        // "we did not reopen anything here". The loop's recovery pass (recoverGated) DOES reopen a container
        // and, on success, re-acts inside it; it stamps `target.revealReplay` before that act, so a recovery
        // act that still fails carries `{replayed:true, ok:true, code, rung}` — the self-describing form of
        // the reopen{ok:true}→act.failed pair that a reader otherwise had to join across two events. Without
        // this, the 22 reveal-required NO_INSTANCE/ALIAS failures were the one class the trail could not
        // explain from the failure row alone (run-log-reviewer, the fix1/dashproof audit).
        revealReplay: target.revealReplay || { replayed: false },
        requests: [], revealed: 0,
    ...
    try {
      ...
      if (err && !err.envelope) {
        ...
        let dismissed = await dismissBlockingOverlay(page, reBlocked?.handle || null);
        // APP-MODAL FALLBACK. dismissBlockingOverlay closes the overlays it recognises; a modal that
        // ignores Escape and exposes only a text Cancel/Close (the live "Schedule a Meeting" shape that
        // covered the Create opener) defeats it, so `dismissed` stays false. On a POINTER-INTERCEPTION
        // failure ONLY — never an ordinary timeout, and never preemptively — close the topmost blocking
        // modal by its own non-committing affordance and let the shared `if (dismissed)` retry fire ONCE.
        // Gated on the interception signal (message shape OR the browser's live hit-test) so it can only
        // fire when a leftover modal is genuinely obscuring the target, not when the next target lives
        // inside one. Under __idle__ (cause reset on the throw), so it forges no request/edge.
        if (!dismissed) {
          const intercepted = isInterceptionTimeout(err)
            || (reBlocked?.handle ? await clickIntercepted(page, reBlocked.handle).catch(() => false) : false);
          if (intercepted) {
            // RECORD THE OBSTRUCTION BEFORE recovering reach (CLASS 1b). A leftover modal covering this
            // control is a defect whether or not the close-and-retry then succeeds; recording it only on
            // failure would erase the finding the moment recovery works — the exact "the interesting event
            // vanished" trap. ADDITIVE node field (never probes → never touches the knowledge ladder / a
            // block code, never identity). Deduped by the obscuring modal so a re-encounter is one finding.
            const obscuredBy = await describeBlockingModal(page, reBlocked?.handle || null);
            const node = graph.elements[target.templateId];
            if (node) {
              const obs = node.obstructions || (node.obstructions = []);
              const sig = obscuredBy ? `${obscuredBy.tag}|${obscuredBy.title || ''}` : 'unknown';
              if (obs.length < 8 && !obs.some((o) => `${o.obscuredBy?.tag}|${o.obscuredBy?.title || ''}` === sig)) {
                obs.push({ obscuredBy, route: target.route || null, probeKind: probeKind || null });
              }
            }
            dismissed = await closeAppModal(page, reBlocked?.handle || null);
          }
        }
        resetTrackerVerdicts(page);
        if (dismissed) {
