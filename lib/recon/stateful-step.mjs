...
export function statefulStep({ page, origin, baselineUrl, ledger, runId, graphPath, exploreAll, stateDir, marker, runCreatedAccount }) {
  ...
  const recordProbe = async (g, target, res, { page: pg, prefill, seen, blocked, domBefore }) => {
  ...
  return async (graph, target) => {
    ...
    await recordProbe(graph, target, res, { page, prefill: prefillUsed, seen: seenBefore, domBefore });
