...
export async function fieldsFor(page, handle, marker) {
  ...
  //
  // `factsKind` carries the field's NATIVE input type so the caller's ownership stamp can skip a SHAPED input
  // (number/date/range/color/…): the zero-width mark makes such a value invalid for its type and the fill
  // throws "Malformed value" — here it would be swallowed by actuateAll's per-field catch and the field
  // silently recorded `skipped`, submitting the form under-filled. This is the same defect step.stampOwnership
  // already fixes on the self-fill path; threading the type closes it on the prefill path too. It is the
  // native type (`isShapedType`'s vocabulary), NOT the actuation `kind` — a native `type=number` actuates as
  // 'fill', so `kind` cannot answer the shaped question.
  return fields.map((f) => ({
    selector: f.selector,
    kind: f.kind || 'fill',
    factsKind: f.type || null,
