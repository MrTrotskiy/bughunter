...
import { findingsOf, renderFindings } from './findings.mjs';
import { outstandingOf, renderOutstanding } from './outstanding.mjs';
...
import { ledger, render as renderLedger } from './coverage-ledger.mjs';
...
function buildReport(graph) {
  ...
  return { ok: true, coverage: stats, instanceCoverage: instanceStats, routeFrontier: routeFrontierStats(graph), routeCoverage: routeCoverageOf(graph), ledger: ledger(graph), routes, requests,
...
function renderText(rep) {
  ...
  // The ledger answers WHY the number above is what it is: an exhaustive partition of every instance,
  // each bucket naming its owner. Printed unconditionally — a coverage figure without it is unactionable.
  if (rep.ledger) { L.push(''); L.push(renderLedger(rep.ledger)); }
...
export function report(opts = {}) {
  ...
  // FINDINGS — the product of the crawl. docs/GOAL.md: an anomalous response is the most valuable thing a
  // crawl can find, and it must carry the control that caused it. Pure read over the graph.
  // THE WORK QUEUE. GOAL.md requires the script to answer total / studied / outstanding at any moment,
  // and to NAME what is left rather than summarising it into a percentage.
  if (opts.outstanding) {
    const o = outstandingOf(graph);
    return opts.json ? o : renderOutstanding(graph, { limit: opts.limit || 40 });
  }
  if (opts.findings) {
    const f = findingsOf(graph);
    return opts.json ? f : renderFindings(graph);
  }
  ...
  // FINDINGS IN THE DEFAULT OUTPUT (L2). The capability existed (`report --findings` → httpAnomalies elevates
  // a 4xx/5xx to a finding keyed to the control that caused it), but it was a SEPARATE command the operator
  // had to know to run — so a `/recon` run's own conclusion never surfaced the repeatable 500 the audit
  // measured, and docs/GOAL.md's "an anomalous response is the most valuable thing a crawl can find" was left
  // to a hand-run report. The default report now HANDS the operator the findings summary (server errors,
  // broken contracts, inert controls) beneath the coverage + causal map. Pure read; `renderFindings` returns
  // "Findings: none recorded." when there are none, so a clean run reads clean.
  if (opts.json) { rep.findings = findingsOf(graph); return rep; }
...
function parseArgs(argv) {
  ...
  for (const a of argv) {
    ...
    else if (a === '--findings') out.findings = true;
    else if (a === '--outstanding') out.outstanding = true;
...
async function main() {
  ...
  try {
    const out = report({ json: args.json, unreached: args.unreached, routeCoverage: args.routeCoverage, completeness: args.completeness, findings: args.findings, outstanding: args.outstanding, limit: args.limit, graphs: args.graphs, run: args.run });
