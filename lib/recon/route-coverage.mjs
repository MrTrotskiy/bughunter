...
export function routeCoverageOf(graph) {
  ...
  // CLIENT-404 (GOAL 1): among the VISITED-BUT-EMPTY routes ONLY, those whose structural contentSig
  // equals the negative-control probe sig (graph.notFoundSig — route-frontier.probeNotFound) are the
  // constant-URL SPA's dead routes rendering the shared Not-Found shell. Matched against a KNOWN-DEAD
  // label, not an internal cluster, so ZERO survive and there is no "which one survives" pick. The
  // EMPTY-ONLY restriction makes false-collapse of a COLLECTED section structurally impossible; a real
  // content-starved section (its own h1, a distinct sig) stays visited-but-empty, never mislabelled.
  const notFoundSig = graph.notFoundSig;
  const client404 = notFoundSig ? empty.filter((n) => n.contentSig && n.contentSig === notFoundSig) : [];
  const c404 = new Set(client404.map((n) => n.url));
  const trueEmpty = empty.filter((n) => !c404.has(n.url));
  ...
  return {
    ...
    collectable,                     // declared minus proven phantoms — the honest coverage-% base
    collected: collected.length,     // reached WITH own content — the honest numerator
