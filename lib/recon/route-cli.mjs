...
// ONE writer per state dir. This driver snapshots a whole page into the graph and saves it — the
// same whole-file overwrite `saveGraph` performs everywhere — and claimed nothing while doing it.
// /recon calls it BETWEEN agent acts, which is sequential by convention only; the lock is what makes
// it true. See state-lock.mjs for what the unenforced version cost.
