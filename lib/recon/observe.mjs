...
import { loadGraph, saveGraph, recordSemantics, markExplored } from '../graph/graph-store.mjs';
...
export function observe(opts) {
  ...
  const acted = opts.acted == null ? true : !(opts.acted === 'false' || opts.acted === false);
  ...
  markExplored(graph, tid);
  saveGraph(graphPath, graph);
  return { ok: true, templateId: tid, explored: true, danger, acted };
}
