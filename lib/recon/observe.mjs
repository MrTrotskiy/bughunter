...
// ONE writer per state dir, here too. This CLI is the agent path's ONLY graph mutator, and it ran
// loadGraph/saveGraph with no claim at all — so a second agent-path process (another `observe`, a
// `route-cli`, a stray `recon-run`) writing the same dir would clobber it silently, exactly the way
// three concurrent crawls once oscillated one run's element count 265 → 73 → 302 between adjacent
// snapshots and cost the session every graph-derived number it had published (state-lock.mjs).
// Sequential invocation is what /recon happens to do; it is not what anything ENFORCED.
export function observe(opts) {
  const stateDir = process.env.BUGHUNTER_STATE_DIR || path.join(process.cwd(), 'state');
  const releaseLock = acquireStateDir(stateDir, { runId: process.env.BUGHUNTER_RUN_ID || null });
  try {
    return observeLocked(opts, stateDir);
  } finally {
    releaseLock();
  }
}

function observeLocked(opts, stateDir) {
