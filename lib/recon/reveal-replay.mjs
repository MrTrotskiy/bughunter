...
import { buildWriteAllowlist, makeFirewallHandler, describeBlock } from './reveal-firewall.mjs';
...
export async function replayRevealPath(page, graph, reveal) {
  ...
  // blocker measured on the live target crawl: 54 templates failed REVEAL_WRITE_BLOCKED, and the
  // blocked requests were the app's OWN same-origin endpoints (update_online_status / addview /
  // listitems) mislabelled off-origin — `reveal-firewall` reads `page.url()` LIVE per request, and a
  ...
  try {
    for (const step of statePath) {
      ...
      if (hardBlock) { // L1: {method,urlPattern,origin,reason} only — an ORIGIN, never the raw url
        throw envelopeError({ code: 'REVEAL_WRITE_BLOCKED', message: `reveal step ${step.templateId} fired a firewall-refused request ${describeBlock(hardBlock)} at replay time — blocked (account unmutated), path refused`, exit: 'VIOLATION' });
      }
