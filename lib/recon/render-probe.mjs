...
// The verdict rule, pure and exported so it is testable without a browser. The Not-Found SIGNATURE decides
// first, before any content count. The old order asked "does it have controls?" first, on the assumption
// that a Not-Found page has none — and the live target's shell renders exactly one, so 18 declared-but-dead
// routes (/groups, /feed, /reports, /engine.io …, all sig 2110f3b4, all own=1/1) were reported `collected`.
// contentSig is structural (text-free, attr-free): matching a random nonexistent path byte-for-byte means
// the same shell rendered, whatever that shell happens to contain.
export function deriveVerdict({ contentInteractive = 0, sigMatchesNotFound = false } = {}) {
  if (sigMatchesNotFound) return 'client-404';
  return contentInteractive > 0 ? 'collected' : 'visited-empty';
}
...
async function classifyRoute(page, rk, origin, notFoundSig) {
  ...
  const verdict = deriveVerdict({ contentInteractive, sigMatchesNotFound });
