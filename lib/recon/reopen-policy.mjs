...
// that question is not answerable lexically: `POST /rawcaster/addnuggetview` is a VIEW COUNTER that fires on
// almost every interaction, yet `endpoint-class` sees the verb `add` at a path boundary and calls it a write,
// so a menu opener, a nav tab and a counter all read as mutating. Meanwhile `POST /rawcaster/influencerlist`
