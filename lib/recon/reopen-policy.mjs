...
// that question is not answerable lexically: `POST /app/addview` is a VIEW COUNTER that fires on
// almost every interaction, yet `endpoint-class` sees the verb `add` at a path boundary and calls it a write,
// so a menu opener, a nav tab and a counter all read as mutating. Meanwhile `POST /app/itemlist`
...
export function admitHop(graph, hop, seen = new Set(), fires = null, targetRoute = null) {
  ...
  // A HOP ON ANOTHER ROUTE CANNOT BE ON THIS TARGET'S PATH. `reopenContainer` navigates to the TARGET's
  // route before walking, so a hop recorded on a different page is not merely useless — it is guaranteed to
  // fail resolution, and the attempt costs a full click timeout before it does. It gets into the path at all
  // because a stateful breadcrumb is a SESSION HISTORY, not a route: the chain resets only on a `routeKey`
  // change, so acts from an earlier page survive into a later page's recorded path.
  // Measured: 9 failures in one run were `Export`@/people replayed after navigating to /projects.
  // Optional and default-off, so a caller that does not know the target route keeps the old behaviour.
  if (targetRoute && route && route !== targetRoute) return { ok: false, code: 'REOPEN_HOP_OFFROUTE' };
  ...
  // "report Report content" and "block Block account" are in neither the destructive nor the communication set,
...
export function reopenAttempts(graph, node, inst, { maxHops = REOPEN_MAX_HOPS, fires = null } = {}) {
  ...
  for (let n = 1; n <= Math.min(maxHops, path.length); n++) {
    ...
    for (const h of hops) {
      verdict = admitHop(graph, h, seen, fires, node?.route || null);
      if (!verdict.ok) break;
