...
function persistentStep({ page, origin, ledger, runId, graphPath }) {
  return async (graph, target) => {
    ...
    try {
      ...
      // Record the FAILED act so the admin timeline shows exactly the "why didn't it reach
      // X?" cases (NOT_VISIBLE / OFF_ORIGIN / NO_INSTANCE / DANGER_FLOOR) — reconLoop's catch
      // otherwise swallows the throw into markUnreachable with no trail entry. Re-throw so the
      // loop still marks it unreachable (coverage unchanged); the agent path already logs this
      // via observe, this is the node-loop backstop.
      if (runId) {
