...
import { installReadOnlyFirewall, summarizeBlocked, resolveWritePolicy } from './read-only-firewall.mjs';
...
function persistentStep({ page, origin, ledger, runId, graphPath, readOnly, huntWrites, marker, runCreatedAccount, huntWindow, createBudget }) {
  return async (graph, target) => {
    ...
    try {
      ...
      // WRITE-HUNT on the NODE-LOOP (additive-only re-scope; decisions.md 2026-07-18 "NODE-LOOP HUNT"): when
      // armed (BUGHUNTER_HUNT + BUGHUNTER_RUN_ID on an authed run), thread the SAME hunt opts whats-new passes
      // so actStep's already-reviewed rails (ownsTarget marker, firewall method-gate, danger-floor auth/account)
      // let SAFE own-content mutations COMMIT. The node-loop has NO LLM judgment, so the one new risk — unbounded
      // create spam — is bounded HERE (not in the shared actStep): once graph.huntCreates hits the operator's
      // BUGHUNTER_HUNT_CREATE_BUDGET this act reverts to read-only (armHunt=false), so the crawl finishes the
      // drain without more writes. Budget=0 (unset) means "no hunt this act". The confirm-modal delete case is
      // a SAFE no-op (actStep clicks one control; no affirmative fires) — delete-with-confirm stays agent-only.
      // A non-positive budget forces hunt OFF for the act (matches the documented "0 disables hunt"): without
      // the `<= 0` arm a `BUGHUNTER_HUNT_CREATE_BUDGET=0` would make `createBudget > 0` false → overBudget
      // never trips → UNBOUNDED writes (the inverse of the intent). `graph.huntCreates` is coerced to a finite
      // number so a corrupt/NaN persisted value can never disable the cap (defaults to 0 → conservative).
      const spent = Number.isFinite(graph.huntCreates) ? graph.huntCreates : 0;
      const overBudget = huntWrites && (createBudget <= 0 || spent >= createBudget);
      const armHunt = !!huntWrites && !overBudget;
      res = await actStep(page, graph, ledger, target, {
        capture, revealPath, refuseMutations: readOnly,
        huntWrites: armHunt,
        marker: armHunt ? marker : null,
        runCreatedAccount,
        huntWindow: armHunt ? huntWindow : null,
        // The node-loop has NO LLM judge, so actStep applies the strict rail: an unowned write from a control
        // inside an ownable item is declined (security H1) — only owned writes + create/composer commit.
        huntStrict: true,
      });
      // Count COMMITTED mutations toward the budget: each non-GET request that got a SUCCESS response (2xx/3xx)
      // is a write that REACHED the server. Counted PER-WRITE (not per-act) so a control firing N writes can't
      // undercount the spam cap (security M1). A firewall-ABORTED write (an unowned delete) has status null and
      // must NOT burn budget. Persisted on the graph (coerced finite) so the count survives a mid-crawl resume.
      if (armHunt) {
        const committedNow = (res.requests || []).filter((r) => {
          const isWrite = String(r.method || 'GET').toUpperCase() !== 'GET';
          return isWrite && Number.isFinite(r.status) && r.status >= 200 && r.status < 400;
        }).length;
        if (committedNow) graph.huntCreates = spent + committedNow;
      }
    } catch (err) {
...
export async function crawl(opts, deps = {}) {
  ...
  // Single source of truth for the write policy (resolveWritePolicy). readOnly is the abort-by-default gate
  // (authed / --read-only / --stateful); huntWrites (a STRICT SUBSET) arms SAFE own-content mutations when the
  // operator opts in (--hunt/BUGHUNTER_HUNT + a run id) on an authed run — OFF by default → byte-identical.
  const { readOnly, huntWrites } = resolveWritePolicy(process.env, {
    readOnly: opts.readOnly, stateful: opts.stateful, hunt: opts.hunt, runId,
  });
  // WRITE-HUNT node-loop (additive-only): the HUNT-<runId> ownership marker + the account-deletion gate +
  // the create-spam budget. Budget default 15 — a runaway backstop for the judgment-less node-loop; 0 disables
  // hunt for the step. The stateful branch stays read-only (statefulStep never threads hunt), so hunt is a
  // STATELESS-node-loop capability only.
  const marker = huntWrites ? huntMarker(runId) : null;
  const runCreatedAccount = !!opts.runCreatedAccount || !!process.env.BUGHUNTER_HUNT_CREATED_ACCOUNT;
  const createBudget = huntWrites ? Number(process.env.BUGHUNTER_HUNT_CREATE_BUDGET || 15) : 0;
  ...
  try {
    ...
    if (opts.stateful) {
      ...
      const step = persistentStep({
        page, origin, ledger, runId, graphPath, readOnly,
        huntWrites, marker, runCreatedAccount, createBudget,
        huntWindow: huntWrites && roFirewall ? roFirewall.hunt : null,
      });
    ...
    // WRITE-HUNT: surface how many mutations committed vs the budget (honest write count; 0/undefined otherwise).
    if (huntWrites) result.huntCreates = { committed: graph.huntCreates || 0, budget: createBudget };
    return result;
  } finally {
...
async function main() {
  ...
  try {
    ...
    const result = await crawl({
      ...
      // WRITE-HUNT (additive-only node-loop): --hunt arms SAFE own-content mutations on an AUTHED run (needs
      // BUGHUNTER_RUN_ID for the marker); --created-account permits account-deletion only if THIS run made it.
      // OFF by default → the crawl stays read-only. Budget via BUGHUNTER_HUNT_CREATE_BUDGET (default 15).
      hunt: args.hunt === true,
