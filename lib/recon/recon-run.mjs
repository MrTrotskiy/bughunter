...
import { loadGraph, saveGraph, markExplored } from '../graph/graph-store.mjs';
...
function persistentStep({ page, origin, ledger, runId, graphPath }) {
  return async (graph, target) => {
    ...
    // "why didn't it reach X?" cases — the PRE-NAV guards below (OFF_ORIGIN / ROUTE_DANGER)
    // AND the actStep throws (NOT_VISIBLE / NO_INSTANCE / DANGER_FLOOR). reconLoop's catch
    // otherwise swallows the throw into markUnreachable with no trail entry.
    ...
    // Reused page: clear stale cross-act initiator verdicts so a path an earlier act click-
    // rooted cannot suppress the timer-rejection of a same-path background poll in THIS act's
    // window (a phantom edge). Safe: the load-burst is already excluded by the token filter.
    resetTrackerVerdicts(page);
    const capture = runId ? makeCapture(runId, target.templateId) : undefined;
    let res;
    try {
      res = await actStep(page, graph, ledger, target, { capture });
    } catch (err) {
      // actStep failure (NOT_VISIBLE / NO_INSTANCE / DANGER_FLOOR). Re-throw so the loop still
      // marks it unreachable (coverage unchanged); the agent path already logs this via observe.
      recordFail(err);
      throw err;
    }
