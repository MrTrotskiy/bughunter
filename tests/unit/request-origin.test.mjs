...
// failure class of a live run undiagnosable. Run `raw1` (108 acts) refused 22 reveal paths with:
//
//   reveal step 76 fired a firewall-refused request POST /rawcaster/addnuggetview (off-origin)
//
...
test('a captured request record carries the HOST and a wall-clock timestamp', () => {
  ...
  lt.onRequest(req('1', 'POST', 'https://api.rawcaster.com/rawcaster/addnuggetview', { wallTime: 1784505600 }));

  const meta = lt.takeResponse('POST', '/rawcaster/addnuggetview');
  assert.ok(meta, 'the request is joined on its pathname as before');
  assert.equal(
    meta.origin, 'https://api.rawcaster.com',
...
test('startedAt is WALL-CLOCK, not the CDP monotonic timestamp used for durations', () => {
  ...
  lt.onRequest(req('1', 'GET', 'https://dev.rawcaster.com/api/me', { timestamp: 10 }));
...
test('urlPattern is UNCHANGED for the same input — the census key is added to, never replaced', () => {
  ...
  assert.equal(toUrlPattern('https://api.rawcaster.com/rawcaster/addnuggetview'), '/rawcaster/addnuggetview');
  ...
  for (let i = 1; i <= 50; i++) patterns.add(toUrlPattern(`https://dev.rawcaster.com/item/${i}`));
  ...
  assert.ok(!toUrlPattern('https://api.rawcaster.com/x').includes('rawcaster.com'), 'urlPattern stays host-less');
});

test('originOf refuses to invent a host for a relative or opaque url', () => {
  assert.equal(originOf('/rawcaster/addnuggetview'), null, 'a relative url has no knowable origin');
  assert.equal(originOf('data:text/plain,hi'), null, 'an opaque origin is null, not the string "null"');
  assert.equal(originOf('https://dev.rawcaster.com:8443/x?y=1'), 'https://dev.rawcaster.com:8443', 'scheme+host+port, no path or query');
});
...
test('the firewall records BOTH origins its off-origin verdict was computed from', async () => {
  const blocked = [];
  const page = { url: () => 'https://dev.rawcaster.com/app/feed' };
  const handler = makeFirewallHandler(page, new Set(), blocked);
  const route = fakeRoute('POST', 'https://api.rawcaster.com/rawcaster/addnuggetview');
  ...
  assert.equal(blocked[0].urlPattern, '/rawcaster/addnuggetview', 'urlPattern unchanged');
  assert.equal(blocked[0].origin, 'https://api.rawcaster.com', 'the REFUSED origin is recorded');
  assert.equal(blocked[0].pageOrigin, 'https://dev.rawcaster.com', 'the origin it was JUDGED AGAINST is recorded');
});

test('the refusal message names the refused origin and the page origin', () => {
  const msg = describeBlock({
    method: 'POST', urlPattern: '/rawcaster/addnuggetview', reason: 'off-origin',
    origin: 'https://api.rawcaster.com', pageOrigin: 'https://dev.rawcaster.com', hard: true,
  });
  assert.ok(
    msg.includes('https://api.rawcaster.com'),
  ...
  assert.ok(msg.includes('https://dev.rawcaster.com'), 'and the page origin the verdict was judged against');
  assert.ok(msg.startsWith('POST /rawcaster/addnuggetview'), 'the method + pattern still lead the message');
