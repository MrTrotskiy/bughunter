...
Status: the keystone (`whats-new`), the Phase-1 loop CORE, AND the semantic recon layer are built and proven green. Loop core: `frontier` + `recon-loop` (pure driver, step+judge injected) + `recon-run` (cold-start runner). Semantic layer (the LLM judge): the `recon` subagent (`.claude/agents/recon.md`, Sonnet) drives thin CLIs — `frontier-cli` (emit the receptive field), `whats-new` (act+capture), `observe` (write purpose/danger/effect + mark explored, gated by a `danger-floor` backstop that refuses firing destructive/logout/payment controls). Explored ⟺ observed in the agent path. Still unbuilt: persistent-session steps (cold-start cannot reach controls behind in-app state), coverage/index surfacing, the Phase-2 designer. Only `recon.md` is native here — the OTHER .claude/agents/*.md are inherited from bughunt-agents and NOT re-wired.
...
- lib/recon/recon-run.mjs — loop runner CLI: baseline → cold-start step → reconLoop
